...
int copyatoms(float *atoms, int count, float zplane, float4 *atominfo) {
  ...
  hipMemcpy((float*)atominfo, atompre, count * 4 * sizeof(float), hipMemcpyHostToDevice);
  CUERR // check and clear any existing errors
...
int main(int argc, char** argv) {
  ...
  int hipdev = 0;
  if (argc == 2) {
    sscanf(argv[1], "%d", &hipdev);
    if (hipdev < 0 || hipdev >= deviceCount) {
      hipdev = 0; 
    }    
  }
  ...
  hipMalloc((void**)&datominfo, sizeof(float4) * MAXATOMS);
  CUERR // check and clear any existing errors
  ...
  hipMemcpy(energy, doutput, volmemsz,  hipMemcpyDeviceToHost);
  CUERR // check and clear any existing errors
