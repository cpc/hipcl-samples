...
const unsigned WALLACE_POOL_SIZE
    = 2048; // number of random numbers per wallace pool. Must be multiple of
            // 16 for alignment
const unsigned WALLACE_POOL_SIZE_MASK = WALLACE_POOL_SIZE - 1;
const unsigned WALLACE_RUNS_PER_THREAD = 2;
const unsigned WALLACE_NUM_THREADS
    = WALLACE_POOL_SIZE
      / (4 * WALLACE_RUNS_PER_THREAD); // each thread transforms four values
                                       // from pool
const unsigned WALLACE_MAX_OUTPUTS_PER_ITERATION
    = 4; // maximum number of outputs per iteration if combine count is 1
#define WALLACE_OUTPUT_COMBINE_COUNT_DEF 1
const unsigned WALLACE_OUTPUT_COMBINE_COUNT
    = WALLACE_OUTPUT_COMBINE_COUNT_DEF; // combine this many pool samples for
                                        // each aggregate (can be 1,2 or 4)
...
const unsigned WALLACE_NUM_RANDOM_NUMBERS_PER_THREAD
    = WALLACE_NUM_OUTPUTS_PER_RUN * WALLACE_RUNS_PER_THREAD
      * WALLACE_MAX_OUTPUTS_PER_ITERATION;
const unsigned WALLACE_TOTAL_NUM_THREADS
    = WALLACE_NUM_BLOCKS * WALLACE_NUM_THREADS;
const unsigned WALLACE_TOTAL_POOL_SIZE
    = WALLACE_POOL_SIZE * WALLACE_NUM_BLOCKS;
...
// EAch block (threads in a block share) needs a chi2 value for each pool
// transform
const unsigned WALLACE_CHI2_VALUES_PER_BLOCK = WALLACE_NUM_OUTPUTS_PER_RUN;
// Chi2 is total number of values needed divided by number of values produced
// per iteration per block.
const unsigned WALLACE_CHI2_COUNT
    = WALLACE_CHI2_VALUES_PER_BLOCK * WALLACE_NUM_BLOCKS;
...
const unsigned TAUSWORTHE_TOTAL_NUM_THREADS
    = TAUSWORTHE_NUM_THREADS * TAUSWORTHE_NUM_BLOCKS;
const unsigned TAUSWORTHE_NUM_SEEDS_PER_GENERATOR = 4;
const unsigned TAUSWORTHE_NUM_SEEDS = TAUSWORTHE_NUM_BLOCKS
                                      * TAUSWORTHE_NUM_THREADS
                                      * TAUSWORTHE_NUM_SEEDS_PER_GENERATOR;
...
const unsigned TAUSWORTHE_TOTAL_SEED_SIZE
    = TAUSWORTHE_SEED_SIZE * TAUSWORTHE_NUM_BLOCKS;
const unsigned TAUSWORTHE_NUM_RANDOM_NUMBERS_PER_THREAD
    = TAUSWORTHE_NUM_OUTPUTS_PER_RUN * 8;
const unsigned TAUSWORTHE_NUM_RANDOM_NUMBERS_PER_BLOCK
    = TAUSWORTHE_NUM_RANDOM_NUMBERS_PER_THREAD * TAUSWORTHE_NUM_THREADS;
const unsigned TAUSWORTHE_OUTPUT_SIZE
    = TAUSWORTHE_NUM_RANDOM_NUMBERS_PER_BLOCK * TAUSWORTHE_NUM_BLOCKS;
...
// EAch block (threads in a block share) needs a chi2 value for each pool
// transform
const unsigned ASIAN_WALLACE_CHI2_VALUES_PER_BLOCK
    = (ASIAN_PATHS_PER_SIM * ASIAN_TIME_STEPS / 4);
// Chi2 is total number of values needed divided by number of values produced
// per iteration per block.
const unsigned ASIAN_WALLACE_CHI2_COUNT
    = WALLACE_NUM_BLOCKS * ASIAN_WALLACE_CHI2_VALUES_PER_BLOCK;
