...
__device__ void initialise_wallace(unsigned &seed, float *globalPool,
  ...
  pool[threadIdx.x + WALLACE_NUM_THREADS] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS];
  pool[threadIdx.x + WALLACE_NUM_THREADS * 2] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS * 2];
  pool[threadIdx.x + WALLACE_NUM_THREADS * 3] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS * 3];
  __syncthreads();
  pool[threadIdx.x + WALLACE_NUM_THREADS * 4] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS * 4];
  pool[threadIdx.x + WALLACE_NUM_THREADS * 5] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS * 5];
  pool[threadIdx.x + WALLACE_NUM_THREADS * 6] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS * 6];
  pool[threadIdx.x + WALLACE_NUM_THREADS * 7] =
      globalPool[offset + threadIdx.x + WALLACE_NUM_THREADS * 7];

  __syncthreads();
}

__device__ void store_wallace(float *globalPool, float *pool) {
...
__device__ void transform_pool(unsigned &m_seed, float *pool) {
...
__device__ float getRandomValue(unsigned n, unsigned loop_counter,
                                float chi2CorrAndScale, float *pool) {
  return pool[__mul24(n, WALLACE_NUM_THREADS) + threadIdx.x] * chi2CorrAndScale;
}

__device__ void generateRandomNumbers_wallace(
    unsigned m_seed, float *chi2Corrections, // move this into constants
    float *globalPool, float *output, float *pool) {
  initialise_wallace(m_seed, globalPool, pool);
  ...
  for (int loop = 0; loop < WALLACE_NUM_OUTPUTS_PER_RUN; loop++) {
    ...
    unsigned intermediate_address =
        __mul24(loop, 8 * WALLACE_TOTAL_NUM_THREADS) +
        __mul24(8 * WALLACE_NUM_THREADS, blockIdx.x) + threadIdx.x;

    if (threadIdx.x == 0)
      pool[WALLACE_CHI2_OFFSET] =
          chi2Corrections[__mul24(blockIdx.x, WALLACE_NUM_OUTPUTS_PER_RUN) +
                          loop];
    __syncthreads();
    float chi2CorrAndScale = pool[WALLACE_CHI2_OFFSET];
    for (int i = 0; i < 8; i++) {
      output[intermediate_address + i * WALLACE_NUM_THREADS] =
          getRandomValue(i, loop, chi2CorrAndScale, pool);
    }

    transform_pool(m_seed, pool);
  }

  store_wallace(globalPool, pool);
}
...
__global__ void rng_wallace(unsigned seed, float *globalPool,
  ...
  HIP_DYNAMIC_SHARED(float, pool);
  generateRandomNumbers_wallace(seed, chi2Corrections, globalPool,
                                generatedRandomNumberPool, (float *)pool);
}
