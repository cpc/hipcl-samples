...
dataType yearFractionCpu(repoDateStruct d1, repoDateStruct d2, int dayCounter) {
...
void getRepoResultsGpuCpu(inArgsStruct inArgs, resultsStruct results,
                          int totNumRuns) {
  for (int repoNum = 0; repoNum < totNumRuns; repoNum++) {
    ...
    repoDateStruct currStartDate =
        advanceDateCpu(inArgs.bond[repoNum].maturityDate, (numLegs - 1) * -6);
    ...
    results.dirtyPrice[repoNum] =
        getDirtyPriceCpu(inArgs, repoNum, cashFlows, numLegs);
    results.accruedAmountSettlement[repoNum] = getAccruedAmountCpu(
        inArgs, inArgs.settlementDate[repoNum], repoNum, cashFlows, numLegs);
    results.accruedAmountDeliveryDate[repoNum] = getAccruedAmountCpu(
    ...
    results.forwardSpotIncome[repoNum] =
        fixedRateBondForwardSpotIncomeCpu(inArgs, repoNum, cashFlows, numLegs);
    ...
    results.repoImpliedYield[repoNum] = getImpliedYieldCpu(
...
dataType getDirtyPriceCpu(inArgsStruct inArgs, int repoNum,
                          cashFlowsStruct cashFlows, int numLegs) {
  dataType currentNotional = bondNotionalCpu();
  return discountingBondEngineCalculateSettlementValueCpu(inArgs, repoNum,
                                                          cashFlows, numLegs) *
         100.0 / currentNotional;
}

dataType getAccruedAmountCpu(inArgsStruct inArgs, repoDateStruct date,
                             int repoNum, cashFlowsStruct cashFlows,
                             int numLegs) {
...
dataType discountingBondEngineCalculateSettlementValueCpu(
  ...
  return cashFlowsNpvCpu(cashFlows, inArgs.discountCurve[repoNum], false,
                         settlementDate, settlementDate, numLegs);
}

dataType bondAccruedAmountCpu(inArgsStruct inArgs, repoDateStruct date,
                              int repoNum, cashFlowsStruct cashFlows,
                              int numLegs) {
  dataType currentNotional = bondNotionalCpu();
  ...
  return bondFunctionsAccruedAmountCpu(inArgs, date, repoNum, cashFlows,
                                       numLegs);
}

dataType bondFunctionsAccruedAmountCpu(inArgsStruct inArgs, repoDateStruct date,
                                       int repoNum, cashFlowsStruct cashFlows,
                                       int numLegs) {
...
dataType cashFlowsAccruedAmountCpu(cashFlowsStruct cashFlows,
                                   ...
                                   repoDateStruct settlementDate, int numLegs,
                                   inArgsStruct inArgs, int repoNum) {
  int legComputeNum =
      cashFlowsNextCashFlowNumCpu(cashFlows, settlementDate, numLegs);

  dataType result = 0.0;

  for (int i = legComputeNum; i < (numLegs); ++i) {
    result += fixedRateCouponAccruedAmountCpu(cashFlows, i, settlementDate,
                                              inArgs, repoNum);
  }
...
dataType fixedRateCouponAccruedAmountCpu(cashFlowsStruct cashFlows, int numLeg,
                                         repoDateStruct d, inArgsStruct inArgs,
                                         int repoNum) {
  if (d.dateSerialNum <=
...
dataType cashFlowsNpvCpu(cashFlowsStruct cashFlows,
                         ...
                         bool includeSettlementDateFlows,
                         repoDateStruct settlementDate, repoDateStruct npvDate,
                         int numLegs) {
  ...
  for (int i = 0; i < numLegs; ++i) {
    ...
    if (!(cashFlowHasOccurredCpu(cashFlows.legs[i].paymentDate,
                                 settlementDate)))
      totalNPV += fixedRateCouponAmountCpu(cashFlows, i) *
                  repoYieldTermStructureDiscountCpu(
...
dataType fixedRateBondForwardSpotIncomeCpu(inArgsStruct inArgs, int repoNum,
                                           cashFlowsStruct cashFlows,
                                           int numLegs) {
  dataType income = 0.0;
...
dataType getImpliedYieldCpu(inArgsStruct inArgs, dataType forwardValue,
                            dataType underlyingSpotValue,
                            dataType spotIncomeIncDiscCurve, int repoNum) {
  dataType t = yearFractionCpu(inArgs.settlementDate[repoNum],
                               inArgs.repoDeliveryDate[repoNum],
                               inArgs.repoCurve[repoNum].dayCounter);
  dataType compoundingFactor =
      forwardValue / (underlyingSpotValue - spotIncomeIncDiscCurve);
  return interestRateImpliedRateCpu(compoundingFactor, SIMPLE_INTEREST, 1.0, t);
}
...
dataType getMarketRepoRateCpu(repoDateStruct d, int comp, dataType freq,
                              repoDateStruct referenceDate, inArgsStruct inArgs,
                              int repoNum) {
  dataType compound =
      1.0 / repoYieldTermStructureDiscountCpu(inArgs.repoCurve[repoNum], d);
...
dataType getCashFlowsYieldCpu(cashFlowsStruct leg, dataType npv, int dayCounter,
                              ...
                              dataType accuracy, int maxIterations,
                              dataType guess) {
  // Brent solver;
  solverStruct solver;
...
dataType fOpCpu(irrFinderStruct f, dataType y, cashFlowsStruct cashFlows,
  ...
  dataType NPV = cashFlowsNpvYieldCpu(cashFlows, yield, false, f.settlementDate,
                                      f.npvDate, numLegs);
...
dataType fDerivativeCpu(irrFinderStruct f, dataType y,
  ...
  return modifiedDurationCpu(cashFlows, yield, f.includeSettlementDateFlows,
