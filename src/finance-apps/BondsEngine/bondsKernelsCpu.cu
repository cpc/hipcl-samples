...
dataType yearFractionCpu(bondsDateStruct d1, bondsDateStruct d2,
                         int dayCounter) {
...
dataType couponNotionalCpu() { return 100.0; }

dataType bondNotionalCpu() { return 100.0; }

dataType fixedRateCouponNominalCpu() { return 100.0; }
...
int getNumCashFlowsCpu(inArgsStruct inArgs, int bondNum) {
  ...
  while (currCashflowDate.dateSerialNum >
         inArgs.bond[bondNum].startDate.dateSerialNum) {
    numCashFlows++;
...
void getBondsResultsCpu(inArgsStruct inArgs, resultsStruct results,
                        int totNumRuns) {
  for (int bondNum = 0; bondNum < totNumRuns; bondNum++) {
    ...
    bondsDateStruct currStartDate =
        advanceDateCpu(inArgs.bond[bondNum].maturityDate, (numLegs - 1) * -6);
    ;
    ...
    results.bondForwardVal[bondNum] =
        getBondYieldCpu(inArgs.bondCleanPrice[bondNum], USE_EXACT_DAY,
                        COMPOUNDED_INTEREST, 2.0, inArgs.currDate[bondNum],
                        ACCURACY, 100, inArgs, bondNum, cashFlows, numLegs);
    inArgs.discountCurve[bondNum].forward = results.bondForwardVal[bondNum];
    results.dirtyPrice[bondNum] =
        getDirtyPriceCpu(inArgs, bondNum, cashFlows, numLegs);
    results.accruedAmountCurrDate[bondNum] = getAccruedAmountCpu(
        inArgs, inArgs.currDate[bondNum], bondNum, cashFlows, numLegs);
    results.cleanPrice[bondNum] =
        results.dirtyPrice[bondNum] - results.accruedAmountCurrDate[bondNum];

    free(cashFlows.legs);
  }
}

dataType getDirtyPriceCpu(inArgsStruct inArgs, int bondNum,
                          cashFlowsStruct cashFlows, int numLegs) {
  dataType currentNotional = bondNotionalCpu();
  return discountingBondEngineCalculateSettlementValueCpu(inArgs, bondNum,
                                                          cashFlows, numLegs) *
         100.0 / currentNotional;
}

dataType getAccruedAmountCpu(inArgsStruct inArgs, bondsDateStruct date,
                             int bondNum, cashFlowsStruct cashFlows,
                             int numLegs) {
  return bondAccruedAmountCpu(inArgs, date, bondNum, cashFlows, numLegs);
}

dataType discountingBondEngineCalculateSettlementValueCpu(
    inArgsStruct inArgs, int bondNum, cashFlowsStruct cashFlows, int numLegs) {
  ...
  return cashFlowsNpvCpu(cashFlows, inArgs.discountCurve[bondNum], false,
                         currDate, currDate, numLegs);
}

dataType bondAccruedAmountCpu(inArgsStruct inArgs, bondsDateStruct date,
                              int bondNum, cashFlowsStruct cashFlows,
                              int numLegs) {
  dataType currentNotional = bondNotionalCpu();
  ...
  return bondFunctionsAccruedAmountCpu(inArgs, date, bondNum, cashFlows,
                                       numLegs);
}

dataType bondFunctionsAccruedAmountCpu(inArgsStruct inArgs,
                                       bondsDateStruct date, int bondNum,
                                       cashFlowsStruct cashFlows, int numLegs) {
  return cashFlowsAccruedAmountCpu(cashFlows, false, date, numLegs, inArgs,
                                   bondNum) *
...
dataType cashFlowsAccruedAmountCpu(cashFlowsStruct cashFlows,
  ...
  int legComputeNum = cashFlowsNextCashFlowNumCpu(cashFlows, currDate, numLegs);

  dataType result = 0.0;

  for (int i = legComputeNum; i < (numLegs); ++i) {
    result += fixedRateCouponAccruedAmountCpu(cashFlows, i, currDate, inArgs,
                                              bondNum);
  }
...
dataType fixedRateCouponAccruedAmountCpu(cashFlowsStruct cashFlows, int numLeg,
                                         bondsDateStruct d, inArgsStruct inArgs,
                                         int bondNum) {
  if (d.dateSerialNum <=
          cashFlows.legs[numLeg].accrualStartDate.dateSerialNum ||
      d.dateSerialNum > inArgs.maturityDate[bondNum].dateSerialNum) {
    return 0.0;
  } else {
...
dataType cashFlowsNpvCpu(cashFlowsStruct cashFlows,
                         ...
                         bondsDateStruct npvDate, int numLegs) {
  npvDate = currDate;
  ...
  for (int i = 0; i < numLegs; ++i) {
    ...
    if (!(cashFlowHasOccurredCpu(cashFlows.legs[i].paymentDate, currDate)))
      totalNPV += fixedRateCouponAmountCpu(cashFlows, i) *
                  bondsYieldTermStructureDiscountCpu(
...
dataType bondsYieldTermStructureDiscountCpu(bondsYieldTermStruct ytStruct,
  ...
  return flatForwardDiscountImplCpu(
      ytStruct.intRate,
      yearFractionCpu(ytStruct.refDate, t, ytStruct.dayCounter));
}
...
dataType interestRateCompoundFactorCpu(intRateStruct intRate,
                                       bondsDateStruct d1, bondsDateStruct d2,
                                       int dayCounter) {
  dataType t = yearFractionCpu(d1, d2, dayCounter);
  return interestRateCompoundFactorCpu(intRate, t);
}

dataType interestRateImpliedRateCpu(dataType compound, int comp, dataType freq,
                                    dataType t) {
  dataType r = 0.0f;
  if (compound == 1.0) {
    r = 0.0;
...
dataType getMarketRepoRateCpu(bondsDateStruct d, int comp, dataType freq,
                              bondsDateStruct referenceDate,
                              inArgsStruct inArgs, int bondNum) {
  dataType compound =
      1.0 / bondsYieldTermStructureDiscountCpu(inArgs.repoCurve[bondNum], d);
...
dataType getBondYieldCpu(dataType cleanPrice, int dc, int comp, dataType freq,
                         bondsDateStruct settlement, dataType accuracy,
  ...
  if (currInArgs.bond[bondNum].startDate.dateSerialNum >
      settlement.dateSerialNum) {
    settlement = currInArgs.bond[bondNum].startDate;
  }

  return getBondFunctionsYieldCpu(cleanPrice, dc, comp, freq, settlement,
                                  accuracy, maxEvaluations, currInArgs, bondNum,
                                  cashFlows, numLegs);
}

dataType getBondFunctionsYieldCpu(dataType cleanPrice, int dc, int comp,
                                  dataType freq, bondsDateStruct settlement,
                                  dataType accuracy, int maxEvaluations,
                                  inArgsStruct currInArgs, int bondNum,
                                  cashFlowsStruct cashFlows, int numLegs) {
  dataType dirtyPrice =
      cleanPrice + bondFunctionsAccruedAmountCpu(currInArgs, settlement,
                                                 bondNum, cashFlows, numLegs);
  dirtyPrice /= 100.0 / bondNotionalCpu();

  return getCashFlowsYieldCpu(cashFlows, dirtyPrice, dc, comp, freq, false,
                              settlement, settlement, numLegs, accuracy,
                              maxEvaluations);
}

dataType getCashFlowsYieldCpu(cashFlowsStruct leg, dataType npv, int dayCounter,
                              int compounding, dataType frequency,
                              bool includecurrDateFlows,
                              bondsDateStruct currDate, bondsDateStruct npvDate,
                              int numLegs, dataType accuracy, int maxIterations,
                              dataType guess) {
  // Brent solver;
  solverStruct solver;
...
dataType solverSolveCpu(solverStruct solver, irrFinderStruct f,
...
dataType cashFlowsNpvYieldCpu(cashFlowsStruct cashFlows, intRateStruct y,
                              bool includecurrDateFlows,
                              bondsDateStruct currDate, bondsDateStruct npvDate,
                              int numLegs) {
  ...
  for (int i = 0; i < numLegs; ++i) {
    ...
    if (first) {
      ...
      discount *= interestRateDiscountFactorCpu(
          y, yearFractionCpu(npvDate, couponDate, y.dayCounter));

    } else {
      discount *= interestRateDiscountFactorCpu(
          y, yearFractionCpu(lastDate, couponDate, y.dayCounter));
    }
...
dataType fOpCpu(irrFinderStruct f, dataType y, cashFlowsStruct cashFlows,
                int numLegs) {
  intRateStruct yield;
  ...
  dataType NPV = cashFlowsNpvYieldCpu(cashFlows, yield, false, f.currDate,
                                      f.npvDate, numLegs);

  return (f.npv - NPV);
}

dataType fDerivativeCpu(irrFinderStruct f, dataType y,
                        cashFlowsStruct cashFlows, int numLegs) {
  intRateStruct yield;
  yield.rate = y;
  yield.dayCounter = f.dayCounter;
  ...
  return modifiedDurationCpu(cashFlows, yield, f.includecurrDateFlows,
                             f.currDate, f.npvDate, numLegs);
}

bool closeCpu(dataType x, dataType y) { return closeCpu(x, y, 42); }
...
dataType modifiedDurationCpu(cashFlowsStruct cashFlows, intRateStruct y,
                             bool includecurrDateFlows,
                             bondsDateStruct currDate, bondsDateStruct npvDate,
                             int numLegs) {
  dataType P = 0.0;
