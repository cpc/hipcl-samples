...
void runBoundsEngine() {
  ...
  // lines int nBondsArray[] = {100, 200, 500, 1000, 2000, 5000, 10000, 20000,
  ...
  // for (numTime=0; numTime < 14; numTime++)
  {
    ...
    inArgsHost.discountCurve =
        (bondsYieldTermStruct *)malloc(numBonds * sizeof(bondsYieldTermStruct));
    inArgsHost.repoCurve =
        (bondsYieldTermStruct *)malloc(numBonds * sizeof(bondsYieldTermStruct));
    inArgsHost.currDate =
        (bondsDateStruct *)malloc(numBonds * sizeof(bondsDateStruct));
    ...
    for (numBond = 0; numBond < numBonds; numBond++) {
      ...
      bondsDateStruct bondIssueDate = intializeDateCpu(
          rand() % 28 + 1, rand() % 12 + 1, 1999 - (rand() % 2));
      // bondsDateStruct bondDatedDate = bondIssueDate;
      bondsDateStruct bondMaturityDate = intializeDateCpu(
          rand() % 28 + 1, rand() % 12 + 1, 2000 + (rand() % 2));

      bondsDateStruct todaysDate =
          intializeDateCpu(bondMaturityDate.day - 1, bondMaturityDate.month,
                           bondMaturityDate.year);
    ...
    resultsHost.accruedAmountCurrDate =
        (dataType *)malloc(numBonds * sizeof(dataType));
    ;
    ...
    resultsFromGpu.accruedAmountCurrDate =
        (dataType *)malloc(numBonds * sizeof(dataType));
    ;
    ...
    hipMalloc((void **)&discountCurveGpu,
              numBonds * sizeof(bondsYieldTermStruct));
    ...
    hipMemcpy(discountCurveGpu, inArgsHost.discountCurve,
              numBonds * sizeof(bondsYieldTermStruct), hipMemcpyHostToDevice);
    hipMemcpy(repoCurveGpu, inArgsHost.repoCurve,
              numBonds * sizeof(bondsYieldTermStruct), hipMemcpyHostToDevice);
    hipMemcpy(currDateGpu, inArgsHost.currDate,
              numBonds * sizeof(bondsDateStruct), hipMemcpyHostToDevice);
    hipMemcpy(maturityDateGpu, inArgsHost.maturityDate,
              numBonds * sizeof(bondsDateStruct), hipMemcpyHostToDevice);
    hipMemcpy(bondCleanPriceGpu, inArgsHost.bondCleanPrice,
              numBonds * sizeof(dataType), hipMemcpyHostToDevice);
    hipMemcpy(bondGpu, inArgsHost.bond, numBonds * sizeof(bondStruct),
              hipMemcpyHostToDevice);
    hipMemcpy(dummyStrikeGpu, inArgsHost.dummyStrike,
              numBonds * sizeof(dataType), hipMemcpyHostToDevice);
    ...
    hipLaunchKernelGGL(getBondsResultsGpu, dim3(grid), dim3(threads), 0, 0,
                       inArgs, results, numBonds);
    hipDeviceSynchronize();

    gettimeofday(&end, NULL);

    hipMemcpy(resultsFromGpu.dirtyPrice, dirtyPriceGpu,
              numBonds * sizeof(dataType), hipMemcpyDeviceToHost);
    hipMemcpy(resultsFromGpu.accruedAmountCurrDate, accruedAmountCurrDateGpu,
              numBonds * sizeof(dataType), hipMemcpyDeviceToHost);
    hipMemcpy(resultsFromGpu.cleanPrice, cleanPriceGpu,
              numBonds * sizeof(dataType), hipMemcpyDeviceToHost);
    hipMemcpy(resultsFromGpu.bondForwardVal, bondForwardValGpu,
              numBonds * sizeof(dataType), hipMemcpyDeviceToHost);
