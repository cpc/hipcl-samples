...
__device__ dataType yearFractionGpu(bondsDateStruct d1, bondsDateStruct d2,
                                    int dayCounter) {
  return dayCountGpu(d1, d2, dayCounter) / 360.0;
}

__device__ int dayCountGpu(bondsDateStruct d1, bondsDateStruct d2,
                           int dayCounter) {
...
__device__ dataType getDirtyPriceGpu(inArgsStruct inArgs, int bondNum,
                                     cashFlowsStruct cashFlows, int numLegs) {
  dataType currentNotional = bondNotionalGpu();
  return discountingBondEngineCalculateSettlementValueGpu(inArgs, bondNum,
                                                          cashFlows, numLegs) *
...
__device__ dataType discountingBondEngineCalculateSettlementValueGpu(
  ...
  return cashFlowsNpvGpu(cashFlows, inArgs.discountCurve[bondNum], false,
...
__device__ dataType bondAccruedAmountGpu(inArgsStruct inArgs,
  ...
  return bondFunctionsAccruedAmountGpu(inArgs, date, bondNum, cashFlows,
                                       numLegs);
}
...
__device__ dataType cashFlowsAccruedAmountGpu(cashFlowsStruct cashFlows,
  ...
  for (i = legComputeNum; i < (numLegs); ++i) {
    result += fixedRateCouponAccruedAmountGpu(cashFlows, i, currDate, inArgs,
                                              bondNum);
  }
  ...
  for (i = 0; i < numLegs; ++i) {
    ...
    if (!(cashFlowHasOccurredGpu(cashFlows.legs[i].paymentDate, currDate)))
      totalNPV += fixedRateCouponAmountGpu(cashFlows, i) *
                  bondsYieldTermStructureDiscountGpu(
...
__device__ dataType bondsYieldTermStructureDiscountGpu(
    bondsYieldTermStruct ytStruct, bondsDateStruct t) {
...
__device__ dataType getMarketRepoRateGpu(bondsDateStruct d, int comp,
                                         dataType freq,
                                         bondsDateStruct referenceDate,
                                         inArgsStruct inArgs, int bondNum) {
  dataType compound =
      1.0 / bondsYieldTermStructureDiscountGpu(inArgs.repoCurve[bondNum], d);
...
__device__ dataType getCashFlowsYieldGpu(
    ...
    dataType frequency, bool includecurrDateFlows, bondsDateStruct currDate,
    bondsDateStruct npvDate, int numLegs, dataType accuracy, int maxIterations,
    dataType guess) {
  // Brent solver;
  solverStruct solver;
...
__device__ dataType fOpGpu(irrFinderStruct f, dataType y,
  ...
  dataType NPV = cashFlowsNpvYieldGpu(cashFlows, yield, false, f.currDate,
                                      f.npvDate, numLegs);
...
__device__ dataType fDerivativeGpu(irrFinderStruct f, dataType y,
  ...
  return modifiedDurationGpu(cashFlows, yield, f.includecurrDateFlows,
...
__global__ void getBondsResultsGpu(inArgsStruct inArgs, resultsStruct results,
                                   int n) {
  ...
  if (bondNum < n) {
    ...
    bondsDateStruct currStartDate =
        advanceDateGpu(inArgs.bond[bondNum].maturityDate, (numLegs - 1) * -6);
    ;
    ...
    results.bondForwardVal[bondNum] =
        getBondYieldGpu(inArgs.bondCleanPrice[bondNum], USE_EXACT_DAY,
    ...
    results.dirtyPrice[bondNum] =
        getDirtyPriceGpu(inArgs, bondNum, cashFlows, numLegs);
    results.accruedAmountCurrDate[bondNum] = getAccruedAmountGpu(
