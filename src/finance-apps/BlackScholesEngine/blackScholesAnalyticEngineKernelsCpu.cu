...
float getDiscountOnDividendYieldCpu(float yearFraction,
  ...
  float intDiscountFactor =
      interestRateDiscountFactorCpu(yearFraction, dividendYieldTermStruct);
  return intDiscountFactor;
}

// device function to get the discount on the risk free rate
float getDiscountOnRiskFreeRateCpu(
    float yearFraction, yieldTermStruct riskFreeRateYieldTermStruct) {
  return interestRateDiscountFactorCpu(yearFraction,
                                       riskFreeRateYieldTermStruct);
}
...
float errorFunctCpu(normalDistStruct normDist, float x) {
  ...
  if (ax < 0.84375) {
    ...
    r = ERROR_FUNCT_pp0 +
        z * (ERROR_FUNCT_pp1 +
             z * (ERROR_FUNCT_pp2 +
                  z * (ERROR_FUNCT_pp3 + z * ERROR_FUNCT_pp4)));
    s = ERROR_FUNCT_one +
        z * (ERROR_FUNCT_qq1 +
             z * (ERROR_FUNCT_qq2 +
                  z * (ERROR_FUNCT_qq3 +
                       z * (ERROR_FUNCT_qq4 + z * ERROR_FUNCT_qq5))));
    y = r / s;
    return x + x * y;
  }
  if (ax < 1.25) {
    ...
    P = ERROR_FUNCT_pa0 +
        s * (ERROR_FUNCT_pa1 +
             s * (ERROR_FUNCT_pa2 +
                  s * (ERROR_FUNCT_pa3 +
                       s * (ERROR_FUNCT_pa4 +
                            s * (ERROR_FUNCT_pa5 + s * ERROR_FUNCT_pa6)))));
    Q = ERROR_FUNCT_one +
        s * (ERROR_FUNCT_qa1 +
             s * (ERROR_FUNCT_qa2 +
                  s * (ERROR_FUNCT_qa3 +
                       s * (ERROR_FUNCT_qa4 +
                            s * (ERROR_FUNCT_qa5 + s * ERROR_FUNCT_qa6)))));
    if (x >= 0)
    ...
    R = ERROR_FUNCT_rb0 +
        s * (ERROR_FUNCT_rb1 +
             s * (ERROR_FUNCT_rb2 +
                  s * (ERROR_FUNCT_rb3 +
                       s * (ERROR_FUNCT_rb4 +
                            s * (ERROR_FUNCT_rb5 + s * ERROR_FUNCT_rb6)))));
    S = ERROR_FUNCT_one +
        s * (ERROR_FUNCT_sb1 +
             s * (ERROR_FUNCT_sb2 +
                  s * (ERROR_FUNCT_sb3 +
...
void initCumNormDistCpu(normalDistStruct &currCumNormDist) {
  ...
  currCumNormDist.normalizationFactor =
      M_SQRT_2 * M_1_SQRTPI / currCumNormDist.sigma;
  currCumNormDist.derNormalizationFactor =
      currCumNormDist.sigma * currCumNormDist.sigma;
...
void initBlackCalcVarsCpu(blackCalcStruct &blackCalculator,
  ...
  blackCalculator.d1 = log(blackCalculator.forward / blackCalculator.strike) /
                           blackCalculator.stdDev +
                       0.5 * blackCalculator.stdDev;
  ...
  blackCalculator.cum_d1 =
      cumNormDistOpCpu(currCumNormDist, blackCalculator.d1);
  blackCalculator.cum_d2 =
      cumNormDistOpCpu(currCumNormDist, blackCalculator.d2);
  blackCalculator.n_d1 =
      cumNormDistDerivCpu(currCumNormDist, blackCalculator.d1);
  blackCalculator.n_d2 =
      cumNormDistDerivCpu(currCumNormDist, blackCalculator.d2);
...
void initBlackCalculatorCpu(blackCalcStruct &blackCalc, payoffStruct payoff,
                            float forwardPrice, float stdDev,
...
float getResultValCpu(blackCalcStruct blackCalculator) {
  float result = blackCalculator.discount *
                 (blackCalculator.forward * blackCalculator.alpha +
                  blackCalculator.x * blackCalculator.beta);
  return result;
}

// global function to retrieve the output value for an option
void getOutValOptionCpu(optionInputStruct *options, float *outputVals,
  ...
  if (optionNum < numVals) {
    ...
    float variance =
        getBlackVolBlackVarCpu(currOption.pricingEngine.blackVolTS);
    float dividendDiscount = getDiscountOnDividendYieldCpu(
        currOption.yearFractionTime, currOption.pricingEngine.dividendTS);
    float riskFreeDiscount = getDiscountOnRiskFreeRateCpu(
        currOption.yearFractionTime, currOption.pricingEngine.riskFreeTS);
    ...
    initBlackCalculatorCpu(blackCalc, currOption.payoff, forwardPrice,
                           sqrtf(variance), riskFreeDiscount);
