...
MonteCarlo::MonteCarlo(const Parameters &params)
  ...
  _tallies = new (ptr1) Tallies(
      ...
      params.simulationParams.energySpectrum, params.simulationParams.nGroups);
  ...
  if (num_particles_on_process <= 0) {
    MC_Fatal_Jump("Not enough particles for each process ( Ranks: %d Num "
                  "Particles: %d ) \n",
  ...
  {
    batch_size = (num_particles_on_process / num_batches) +
                 ((num_particles_on_process % num_batches == 0) ? 0 : 1);
  } else // batch size explicatly set - use to find num_batches
  {
    num_batches = num_particles_on_process / batch_size +
                  ((num_particles_on_process % batch_size == 0) ? 0 : 1);
  }
  ...
  // Previous definition was not enough extra space for some reason? need to
  // determine why still
  ...
  _particleVaultContainer = new (ptr6)
      ParticleVaultContainer(batch_size, num_batches, num_extra_vaults);
#else
  particle_buffer = new MC_Particle_Buffer(this, batch_size);
  _particleVaultContainer =
      new ParticleVaultContainer(batch_size, num_batches, num_extra_vaults);
#endif
}
