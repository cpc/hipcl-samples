...
void MC_Verify_Thread_Zero(char const *const file, int line) {
  ...
  if (thread_id != 0) {
    ...
    fprintf(stderr,
            "Fatal Error: %s:%d MPI Routine called by thread other than zero."
            "\n\tMPI Process %d, Thread %d",
            file, line, mpi_rank, thread_id);
...
//----------------------------------------------------------------------------------------------------------------------
// Converts a format string into a c++ string. Parameters are the same as
// printf.
//----------------------------------------------------------------------------------------------------------------------
std::string MC_String(const char fmt[], ...) {
  ...
  // Bump up chars_needed (if necessary) so that we allocate according to our
  // byte alignment. This is currently 16 bytes, so allocated 16, 32 48, etc.
  // bytes at a time.
