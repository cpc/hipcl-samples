...
#define MC_CALLOC(A, N1, TYPE)                                                 \
  if (N1) {                                                                    \
    A = (TYPE *)calloc((N1), sizeof(TYPE));                                    \
  } else {                                                                     \
    A = NULL;                                                                  \
  }
#define MC_MALLOC(A, N1, TYPE)                                                 \
  if (N1) {                                                                    \
    A = (TYPE *)malloc((N1) * sizeof(TYPE));                                   \
  } else {                                                                     \
    A = NULL;                                                                  \
  }
#define MC_NEW_ARRAY(A, N1, TYPE)                                              \
  if (N1) {                                                                    \
    A = new TYPE[N1];                                                          \
  } else {                                                                     \
    A = NULL;                                                                  \
  }
#define MC_REALLOC(a, b, c)                                                    \
  { qs_assert(false); }
#define MC_FREE(A)                                                             \
  if (A != NULL) {                                                             \
    free(A);                                                                   \
...
#define MC_DELETE_ARRAY(A)                                                     \
  if (A != NULL) {                                                             \
    delete[] A;                                                                \
    A = NULL;                                                                  \
  }
#define MC_MEMCPY(a, b, c)                                                     \
  { qs_assert(false); }
#define MC_FABS(x) ((x) < 0 ? -(x) : (x))

#define MC_Fatal_Jump(...)                                                     \
  { qs_assert(false); }
