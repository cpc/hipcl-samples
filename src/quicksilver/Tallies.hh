...
public:
  ...
  Balance()
      : _absorb(0), _census(0), _escape(0), _collision(0), _end(0), _fission(0),
        _produce(0), _scatter(0), _start(0), _source(0), _rr(0), _split(0),
        _numSegments(0) {}
  ...
  void PrintHeader() {
    Print0("%12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s",
           "start", "source", "rr", "split", "absorb", "scatter", "fission",
           "produce", "collisn", "escape", "census", "num_seg");
  }

  void Print() {
    Print0("%12" PRIu64 " %12" PRIu64 " %12" PRIu64 " %12" PRIu64 " %12" PRIu64
           " %12" PRIu64 " %12" PRIu64 " %12" PRIu64 " %12" PRIu64 " %12" PRIu64
           " %12" PRIu64 " %12" PRIu64 "",
           _start, _source, _rr, _split, _absorb, _scatter, _fission, _produce,
           _collision, _escape, _census, _numSegments);
  }

  void Reset() {
    _absorb = _census = _escape = _collision = _end = _fission = _produce =
        _scatter = _start = _source = _rr = _split = _numSegments = 0;
  }
...
public:
  ...
  ScalarFluxCell(double *storage, int size) : _group(storage), _size(size) {
...
public:
  ...
  CellTallyTask(MC_Domain *domain) {
    ...
    _cell.Open();
    for (int cellIndex = 0; cellIndex < domain->cell_state.size();
         cellIndex++) {
      _cell.push_back(0.0);
    }
    _cell.Close();
...
public:
  ...
  ScalarFluxTask(MC_Domain *domain, int numGroups) {
    if (_cell.capacity() == 0) {
      _cell.reserve(domain->cell_state.size(), VAR_MEM);
      _scalarFluxCellStorage.setCapacity(domain->cell_state.size() * numGroups,
                                         VAR_MEM);
    }
  ...
  void Add(ScalarFluxTask &scalarFluxTask) {
    ...
    for (int cellIndex = 0; cellIndex < _cell.size(); cellIndex++) {
      for (int groupIndex = 0; groupIndex < numGroups; groupIndex++) {
        _cell[cellIndex]._group[groupIndex] +=
            scalarFluxTask._cell[cellIndex]._group[groupIndex];
      }
    }
  }
...
public:
  ...
  CellTallyDomain(MC_Domain *domain, int cellTally_replications) {
    ...
    _task.Open();
    for (int task_index = 0; task_index < cellTally_replications;
         task_index++) {
...
public:
  ...
  Tallies(int balRep, int fluxRep, int cellRep, std::string spectrumName,
          int spectrumSize)
      : _balanceCumulative(), _balanceTask(), _scalarFluxDomain(),
        _num_balance_replications(balRep), _num_flux_replications(fluxRep),
        _num_cellTally_replications(cellRep),
        _spectrum(spectrumName, spectrumSize) {}

  HOST_DEVICE_CUDA
  int GetNumBalanceReplications() { return _num_balance_replications; }

  HOST_DEVICE_CUDA
  int GetNumFluxReplications() { return _num_flux_replications; }

  HOST_DEVICE_CUDA
  int GetNumCellTallyReplications() { return _num_cellTally_replications; }

  ~Tallies() {}

  void InitializeTallies(MonteCarlo *monteCarlo, int balance_replications,
                         int flux_replications, int cell_replications);
  ...
  HOST_DEVICE_CUDA
  void TallyScalarFlux(double value, int domain, int task, int cell,
                       int group) {
    ATOMIC_ADD(_scalarFluxDomain[domain]._task[task]._cell[cell]._group[group],
               value);
  }
