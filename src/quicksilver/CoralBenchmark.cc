...
//
//  Coral Benchmark tests are only relavent/tested for the  coral benchmark
//  input deck
//
...
void BalanceRatioTest(MonteCarlo *monteCarlo, Parameters &params) {
  fprintf(stdout, "\n");
  fprintf(stdout, "Testing Ratios for Absorbtion, Fission, and Scattering are "
                  "maintained\n");
  ...
  double Absorb2Scatter =
      std::abs((absorb / absorbRatio) * (scatterRatio / scatter) - 1);
  double Absorb2Fission =
      std::abs((absorb / absorbRatio) * (fissionRatio / fission) - 1);
  double Scatter2Absorb =
      std::abs((scatter / scatterRatio) * (absorbRatio / absorb) - 1);
  double Scatter2Fission =
      std::abs((scatter / scatterRatio) * (fissionRatio / fission) - 1);
  double Fission2Absorb =
      std::abs((fission / fissionRatio) * (absorbRatio / absorb) - 1);
  double Fission2Scatter =
      std::abs((fission / fissionRatio) * (scatterRatio / scatter) - 1);

  bool pass = true;

  if (Absorb2Scatter > tolerance)
    pass = false;
  if (Absorb2Fission > tolerance)
    pass = false;
  if (Scatter2Absorb > tolerance)
    pass = false;
  if (Scatter2Fission > tolerance)
    pass = false;
  if (Fission2Absorb > tolerance)
    pass = false;
  if (Fission2Scatter > tolerance)
    pass = false;

  if (pass) {
    fprintf(stdout,
            "PASS:: Absorption / Fission / Scatter Ratios maintained with %g%% "
            "tolerance\n",
            tolerance * 100.0);
  } else {
    fprintf(stdout,
            "FAIL:: Absorption / Fission / Scatter Ratios NOT maintained with "
            "%g%% tolerance\n",
            tolerance * 100.0);
...
void BalanceEventTest(MonteCarlo *monteCarlo) {
  ...
  if (pass) {
    ...
    fprintf(stdout,
            " FAIL:: Collision to Facet Crossing Ratio balanced NOT maintained "
            "within %g%% tolerance\n",
            tolerance);
...
void MissingParticleTest(MonteCarlo *monteCarlo) {
  ...
  gains =
      balTally._start + balTally._source + balTally._produce + balTally._split;
  losses = balTally._absorb + balTally._census + balTally._escape +
           balTally._rr + balTally._fission;

  if (gains == losses) {
    fprintf(stdout, "PASS:: No Particles Lost During Run\n");
  } else {
    fprintf(stdout, "FAIL:: Particles Were Lost During Run, test for done "
                    "should have failed\n");
  }
}
...
void FluenceTest(MonteCarlo *monteCarlo) {
  ...
  for (int domainIndex = 0; domainIndex < numDomains; domainIndex++) {
    ...
    for (int cellIndex = 0; cellIndex < numCells; cellIndex++) {
      local_sum += monteCarlo->_tallies->_fluence._domain[domainIndex]->getCell(
          cellIndex);
    }
    ...
    for (int cellIndex = 0; cellIndex < numCells; cellIndex++) {
      double cellValue =
          monteCarlo->_tallies->_fluence._domain[domainIndex]->getCell(
              cellIndex);
      double percent_diff =
          (((cellValue > average) ? cellValue - average : average - cellValue) /
           ((cellValue + average) / 2.0)) *
          100;
  ...
  mpiAllreduce(&max_diff, &max_diff_global, 1, MPI_DOUBLE, MPI_MAX,
  ...
  if (monteCarlo->processor_info->rank == 0) {
    if (max_diff_global > percent_tolerance) {
      fprintf(
          stdout,
          "FAIL:: Fluence not homogenous across cells within %g%% tolerance\n",
          percent_tolerance);
      fprintf(stdout, "\tTry running more particles or more cycles to see if "
                      "Max Percent Difference goes down.\n");
      fprintf(stdout, "\tCurrent Max Percent Diff: %4.1f%%\n", max_diff_global);
    } else {
      fprintf(stdout,
              "PASS:: Fluence is homogenous across cells with %g%% tolerance\n",
              percent_tolerance);
    }
  }
}
