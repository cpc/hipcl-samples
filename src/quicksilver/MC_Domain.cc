...
int nodeIndirect[24][3] = {
    {1, 3, 8},  {3, 7, 8},  {7, 5, 8},  {5, 1, 8},  {0, 4, 9},  {4, 6, 9},
    {6, 2, 9},  {2, 0, 9},  {3, 2, 10}, {2, 6, 10}, {6, 7, 10}, {7, 3, 10},
    {0, 1, 11}, {1, 5, 11}, {5, 4, 11}, {4, 0, 11}, {4, 5, 12}, {5, 7, 12},
    {7, 6, 12}, {6, 4, 12}, {0, 2, 13}, {2, 3, 13}, {3, 1, 13}, {1, 0, 13}};

int opposingFacet[24] = {7, 6,  5,  4, 3,  2,  1,  0,  12, 15, 14, 13,
                         8, 11, 10, 9, 20, 23, 22, 21, 16, 19, 18, 17};
...
void buildCells(
    qs_vector<MC_Facet_Adjacency_Cell> &cell,
    BulkStorage<MC_Facet_Adjacency> &facetStore, BulkStorage<int> &pointStore,
    const map<Long64, int> &nodeIndexMap, const qs_vector<int> &nbrDomain,
    const MeshPartition &partition, const GlobalFccGrid &grid,
    const qs_vector<MC_Subfacet_Adjacency_Event::Enum> &boundaryCondition);

void makeFacet(MC_Facet_Adjacency &facet, const MC_Location &location,
               int *nodeIndex, const vector<FaceInfo> &faceInfo);

string findMaterial(const Parameters &params, const MC_Vector &rr);

qs_vector<MC_Subfacet_Adjacency_Event::Enum>
getBoundaryCondition(const Parameters &params);
} // namespace

MC_Mesh_Domain::MC_Mesh_Domain(
    const MeshPartition &meshPartition, const GlobalFccGrid &grid,
  ...
  buildCells(_cellConnectivity, _connectivityFacetStorage,
             _connectivityPointStorage, nodeIndexMap, _nbrDomainGid,
             meshPartition, grid, boundaryCondition);
  ...
  for (auto iter = partition.begin(); iter != partition.end(); ++iter) {
    ...
    for (unsigned ii = 0; ii < 6; ++ii) {
      ...
      if (faceNbr[ii] == iCellGid)
        ...
        //                if (jCellInfo._domainIndex != domainIndex &&
        //                jCellInfo._foreman == foreman)
        //                   faceInfo[ii]._event =
        //                   MC_Subfacet_Adjacency_Event::Transit_On_Processor;
        //                if (jCellInfo._foreman != foreman)
        //                   faceInfo[ii]._event =
...
MC_Domain::MC_Domain(const MeshPartition &meshPartition,
                     const GlobalFccGrid &grid, const DecompositionObject &ddc,
  ...
  _cachedCrossSectionStorage.setCapacity(cell_state.size() * numEnergyGroups,
                                         VAR_MEM);

  for (unsigned ii = 0; ii < cell_state.size(); ++ii) {
    cell_state[ii]._volume = cellVolume(mesh._cellConnectivity[ii], mesh._node);
    ...
    cell_state[ii]._total =
        _cachedCrossSectionStorage.getBlock(numEnergyGroups);
    for (unsigned jj = 0; jj < numEnergyGroups; ++jj)
      cell_state[ii]._total[jj] = 0.;

    int numIsos = static_cast<int>(
        materialDatabase._mat[cell_state[ii]._material]._iso.size());
    ...
    MC_Vector cellCenter =
        findCellCenter(mesh._cellConnectivity[ii], mesh._node);
...
getBoundaryCondition(const Parameters &params) {
  ...
  if (params.simulationParams.boundaryCondition == "reflect")
    bc = qs_vector<MC_Subfacet_Adjacency_Event::Enum>(
        6, MC_Subfacet_Adjacency_Event::Boundary_Reflection);
  else if (params.simulationParams.boundaryCondition == "escape")
    bc = qs_vector<MC_Subfacet_Adjacency_Event::Enum>(
        6, MC_Subfacet_Adjacency_Event::Boundary_Escape);
