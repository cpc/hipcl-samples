...
HOST_DEVICE
static inline unsigned int MC_Find_Min(const double *array, int num_elements);
HOST_DEVICE_END

//--------------------------------------------------------------------------------------------------
//  Routine MC_Segment_Outcome determines whether the next segment of the
//  particle's trajectory will result in:
...
MC_Segment_Outcome(MonteCarlo *monteCarlo, MC_Particle &mc_particle,
...
#if 1
      printf(
          " MC_Segment_Outcome: mc_particle.num_mean_free_paths > -900.0 \n");
#else
  ...
  double macroscopic_total_cross_section = weightedMacroscopicCrossSection(
      monteCarlo, 0, mc_particle.domain, mc_particle.cell,
  ...
  if (force_collision) {
    distance[MC_Segment_Outcome_type::Collision] =
        PhysicalConstants::_smallDouble;
  } else {
    distance[MC_Segment_Outcome_type::Collision] =
  ...
  distance[MC_Segment_Outcome_type::Census] =
      particle_speed * mc_particle.time_to_census;
  ...
  nearest_facet = MCT_Nearest_Facet(
      &mc_particle, location, mc_particle.coordinate, direction_cosine,
      distance_threshold, current_best_distance, new_segment, monteCarlo);
  ...
  mc_particle.num_mean_free_paths -=
      mc_particle.segment_path_length / mc_particle.mean_free_path;

  // Before using segment_outcome as an index, verify it is valid
  if (segment_outcome < 0 ||
      segment_outcome >= MC_Segment_Outcome_type::Max_Number) {
    MC_Fatal_Jump("segment_outcome '%d' is invalid\n", (int)segment_outcome);
  }

  MC_Tally_Event::Enum
      SegmentOutcome_to_LastEvent[MC_Segment_Outcome_type::Max_Number] = {
  ...
  // Move particle to end of segment, accounting for some physics processes
  // along the segment.

  // Project the particle trajectory along the segment path length.
  mc_particle.Move_Particle(mc_particle.direction_cosine,
                            mc_particle.segment_path_length);
  ...
  monteCarlo->_tallies->TallyScalarFlux(
      mc_particle.segment_path_length * mc_particle.weight, mc_particle.domain,
