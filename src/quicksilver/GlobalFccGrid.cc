...
GlobalFccGrid::GlobalFccGrid(int nx, int ny, int nz, double lx, double ly,
                             double lz)
    : _nx(nx), _ny(ny), _nz(nz), _lx(lx), _ly(ly), _lz(lz),
      _cellTupleToIndex(nx, ny, nz), _cellIndexToTuple(nx, ny, nz),
...
void GlobalFccGrid::getFaceNbrGids(Long64 cellGid,
                                   vector<Long64> &nbrCellGid) const {
