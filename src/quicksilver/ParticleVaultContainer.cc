...
ParticleVaultContainer::ParticleVaultContainer(uint64_t vault_size,
                                               uint64_t num_vaults,
                                               uint64_t num_extra_vaults)
    : _vaultSize(vault_size), _numExtraVaults(num_extra_vaults),
      _extraVaultIndex(0), _processingVault(num_vaults),
      _processedVault(num_vaults), _extraVault(num_extra_vaults, VAR_MEM) {
  ...
  for (uint64_t vault = 0; vault < num_vaults; vault++) {
    ...
    _processedVault[vault] = MemoryControl::allocate<ParticleVault>(1, VAR_MEM);
    _processedVault[vault]->reserve(vault_size);
  }

  // Allocate and reserve space for particles for each extra vault
  for (uint64_t e_vault = 0; e_vault < num_extra_vaults; e_vault++) {
    // Allocate Extra Vault
    _extraVault[e_vault] = MemoryControl::allocate<ParticleVault>(1, VAR_MEM);
...
ParticleVaultContainer::~ParticleVaultContainer() {
...
ParticleVault *
ParticleVaultContainer::getTaskProcessingVault(uint64_t vaultIndex) {
...
ParticleVault *
ParticleVaultContainer::getTaskProcessedVault(uint64_t vaultIndex) {
  //   qs_assert(vaultIndex >= 0);
...
uint64_t ParticleVaultContainer::getFirstEmptyProcessedVault() {
  uint64_t index = 0;
...
//--------------------------------------------------------------
HOST_DEVICE
SendQueue *ParticleVaultContainer::getSendQueue() { return this->_sendQueue; }
HOST_DEVICE_END
...
uint64_t ParticleVaultContainer::sizeProcessing() {
  uint64_t sum_size = 0;
...
uint64_t ParticleVaultContainer::sizeProcessed() {
  uint64_t sum_size = 0;
...
uint64_t ParticleVaultContainer::sizeExtra() {
  uint64_t sum_size = 0;
...
void ParticleVaultContainer::collapseProcessing() {
  ...
  while (fill_vault_index < from_vault_index) {
    if (_processingVault[fill_vault_index]->size() == this->_vaultSize) {
      ...
      if (this->_processingVault[from_vault_index]->size() == 0) {
        ...
        uint64_t fill_size =
            this->_vaultSize - this->_processingVault[fill_vault_index]->size();

        this->_processingVault[fill_vault_index]->collapse(
            fill_size, this->_processingVault[from_vault_index]);
      }
    }
  }
}
...
void ParticleVaultContainer::collapseProcessed() {
  ...
  while (fill_vault_index < from_vault_index) {
    if (_processedVault[fill_vault_index]->size() == this->_vaultSize) {
      ...
      if (this->_processedVault[from_vault_index]->size() == 0) {
        ...
        uint64_t fill_size =
            this->_vaultSize - this->_processedVault[fill_vault_index]->size();

        this->_processedVault[fill_vault_index]->collapse(
            fill_size, this->_processedVault[from_vault_index]);
      }
    }
  }
}
...
void ParticleVaultContainer::swapProcessingProcessedVaults() {
  ...
  while (need_to_swap) {
    std::swap(this->_processedVault[processed_vault],
              this->_processingVault[processed_vault]);
    processed_vault++;
...
void ParticleVaultContainer::addProcessingParticle(MC_Base_Particle &particle,
                                                   uint64_t &fill_vault_index) {
...
//--------------------------------------------------------------
HOST_DEVICE
void ParticleVaultContainer::addExtraParticle(MC_Particle &particle) {
  uint64_cu index = 0;
...
void ParticleVaultContainer::cleanExtraVaults() {
  uint64_t size_extra = this->sizeExtra();
  if (size_extra > 0) {
    uint64_t num_extra = (size_extra / this->_vaultSize) +
                         ((size_extra % this->_vaultSize == 0) ? 0 : 1);
    ...
    while (extra_index < num_extra) {
      ...
      if (this->_extraVault[extra_index]->size() == 0) {
        ...
        if (processing_index == this->_processingVault.size()) {
          ...
          if (this->_processingVault[processing_index]->size() ==
              this->_vaultSize) {
            processing_index++;
          } else {
            uint64_t fill_size =
                this->_vaultSize -
                this->_processingVault[processing_index]->size();

            this->_processingVault[processing_index]->collapse(
                fill_size, this->_extraVault[extra_index]);
          }
        }
      }
    }
