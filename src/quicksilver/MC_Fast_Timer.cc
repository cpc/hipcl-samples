...
void MC_Fast_Timer_Container::Print_Last_Cycle_Heading(int mpi_rank) {
  ...
  if (mpi_rank == 0) {
    fprintf(stdout, "\n%-25s %12s %12s %12s %12s %12s %12s\n", "Timer",
            "Last Cycle", "Last Cycle", "Last Cycle", "Last Cycle",
            "Last Cycle", "Last Cycle");
    fprintf(stdout, "%-25s %12s %12s %12s %12s %12s %12s\n", "Name", "number",
            "microSecs", "microSecs", "microSecs", "microSecs", "Efficiency");
    fprintf(stdout, "%-25s %12s %12s %12s %12s %12s %12s\n", "", "of calls",
            "min", "avg", "max", "stddev", "Rating");
  }
}
...
void MC_Fast_Timer_Container::Cumulative_Report(int mpi_rank, int num_ranks,
                                                MPI_Comm comm_world,
  ...
  std::vector<uint64_t> std_dev_use(
      num_ranks); // used to calculate standard deviation

  for (int timer_index = 0; timer_index < MC_Fast_Timer::Num_Timers;
       timer_index++) {
    cumulativeClock[timer_index] = this->timers[timer_index].cumulativeClock;
  }

  mpiReduce(&cumulativeClock[0], &max_clock[0], MC_Fast_Timer::Num_Timers,
            MPI_UINT64_T, MPI_MAX, 0, comm_world);
  mpiReduce(&cumulativeClock[0], &min_clock[0], MC_Fast_Timer::Num_Timers,
            MPI_UINT64_T, MPI_MIN, 0, comm_world);
  mpiReduce(&cumulativeClock[0], &sum_clock[0], MC_Fast_Timer::Num_Timers,
            MPI_UINT64_T, MPI_SUM, 0, comm_world);

  this->Print_Cumulative_Heading(mpi_rank);

  for (int timer_index = 0; timer_index < MC_Fast_Timer::Num_Timers;
       timer_index++) {
    mpiGather(&cumulativeClock[timer_index], 1, MPI_UINT64_T, &std_dev_use[0],
              1, MPI_UINT64_T, 0, comm_world);
    ...
    if (mpi_rank == 0) {
      fprintf(stdout, "%-25s %12lu %12.3e %12.3e %12.3e %12.3e %12.2f\n",
              ...
              (double)min_clock[timer_index], (double)ave_clock,
...
void MC_Fast_Timer_Container::Last_Cycle_Report(int report_time, int mpi_rank,
                                                int num_ranks,
                                                MPI_Comm comm_world) {
#ifdef DISABLE_TIMERS
  return;
  ...
  if (report_time == 1) {
    ...
    mpiReduce(&lastCycleClock[0], &max_clock[0], MC_Fast_Timer::Num_Timers,
              MPI_UINT64_T, MPI_MAX, 0, comm_world);
    mpiReduce(&lastCycleClock[0], &min_clock[0], MC_Fast_Timer::Num_Timers,
              MPI_UINT64_T, MPI_MIN, 0, comm_world);
    mpiReduce(&lastCycleClock[0], &sum_clock[0], MC_Fast_Timer::Num_Timers,
              MPI_UINT64_T, MPI_SUM, 0, comm_world);
    ...
    for (int timer_index = 0; timer_index < MC_Fast_Timer::Num_Timers;
         timer_index++) {
      mpiGather(&lastCycleClock[timer_index], 1, MPI_UINT64_T, &std_dev_use[0],
                1, MPI_UINT64_T, 0, comm_world);
