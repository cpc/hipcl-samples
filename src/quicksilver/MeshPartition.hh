...
struct CellInfo {
  CellInfo() : _domainGid(-1), _foreman(-1), _domainIndex(-1), _cellIndex(-1) {}
  CellInfo(int domainGid, int foreman, int domainIndex, int cellIndex)
      : _domainGid(domainGid), _foreman(foreman), _domainIndex(domainIndex),
...
public:
  ...
  void addCell(Long64 cellGid, const CellInfo &cellInfo) {
    _cellInfoMap[cellGid] = cellInfo;
  }
