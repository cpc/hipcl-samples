...
class GlobalFccGrid {
public:
  GlobalFccGrid(int nx, int ny, int nz, double lx, double ly, double lz);
  ...
  Tuple cellIndexToTuple(Long64 iCell) const {
    return _cellIndexToTuple(iCell);
  }
  Long64 cellTupleToIndex(const Tuple &tt) const {
    return _cellTupleToIndex(tt);
  }
