...
MpiCommObject::MpiCommObject(const MPI_Comm &comm,
...

{
  ...
  // post recvs
  {
    ...
    for (unsigned ii = 0; ii < nbrDomain.size(); ++ii) {
      ...
      mpiIrecv(recvPtr, nRecv, datatype, sourceRank, xTag, _comm,
               request + 2 * ii);
      recvPtr += nRecv;
    }
  }
  ...
  // assemble buffers and send
  {
    ...
    for (unsigned ii = 0; ii < nbrDomain.size(); ++ii) {
      ...
      mpiIsend(sendPtr, nSend, datatype, targetRank, xTag, _comm,
               request + (2 * ii) + 1);
      sendPtr += nSend;
    }
  }
...
void MpiCommObject::exchange(vector<FacetPair> sendBuf,
...

  {
    for (int ii = 0; ii < nRecv; ++ii) {
      ...
      mpiIrecv(recvPtr, nItems, datatype, sourceRank, xTag, _comm,
               request + 2 * ii);
    }
  }
  {
    for (int ii = 0; ii < nSend; ++ii) {
      ...
      mpiIsend(sendPtr, nItems, datatype, targetRank, xTag, _comm,
               request + (2 * ii) + 1);
    }
  }
