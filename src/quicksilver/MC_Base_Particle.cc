...
#define MCP_DATA_MEMBER_OLD(member, buffer, index, mode)                       \
  {                                                                            \
    if (mode == MC_Data_Member_Operation::Count) {                             \
      (index)++;                                                               \
    } else if (mode == MC_Data_Member_Operation::Pack) {                       \
      buffer[(index)++] = (member);                                            \
    } else if (mode == MC_Data_Member_Operation::Unpack) {                     \
      member = buffer[(index)++];                                              \
    } else if (mode == MC_Data_Member_Operation::Reset) {                      \
      (index)++;                                                               \
      member = 0;                                                              \
    }                                                                          \
  }
...
#define MCP_DATA_MEMBER_LONG_TO_CHAR8(member, buffer, index, mode)             \
  {                                                                            \
    if (mode == MC_Data_Member_Operation::Count) {                             \
      ...
      MC_Long_To_Char8(&member, &buffer[(index)]);                             \
      (index) += 8;                                                            \
    } else if (mode == MC_Data_Member_Operation::Unpack) {                     \
      MC_Char8_To_Long(&member, &buffer[(index)]);                             \
      (index) += 8;                                                            \
    } else if (mode == MC_Data_Member_Operation::Reset) {                      \
      (index) += 8;                                                            \
      member = 0;                                                              \
...
void MC_Char8_To_Long(uint64_t *long_out, char char_in[8]) {
  *long_out = 0;
  ...
  for (int char_index = 0; char_index < 8; char_index++) {
    *long_out =
        *long_out | (unsigned char)char_in[char_index]; // OR in next byte
    if (char_index < 7) {
...
void MC_Long_To_Char8(const uint64_t *long_in, char char_out[8]) {
  uint64_t long_tmp;
...
//  different modes.  This is so that the exact same code does the counting,
//  packing and unpacking so they  will always stay synchronized and the
//  communication will happen correctly. Also, when the data structure changes,
//  you only have to change this one place.
//
//----------------------------------------------------------------------------------------------------------------------
void MC_Base_Particle::Serialize(int *int_data, double *float_data,
                                 char *char_data, int &int_index,
                                 int &float_index, int &char_index,
  ...
  MCP_DATA_MEMBER_LONG_TO_CHAR8(random_number_seed, char_data, char_index,
                                mode);
  MCP_DATA_MEMBER_LONG_TO_CHAR8(identifier, char_data, char_index, mode);

  MCP_DATA_MEMBER_CAST_OLD(last_event, int_data, int_index, mode,
                           MC_Tally_Event::Enum);
...
//----------------------------------------------------------------------------------------------------------------------
//  Updates the num base counts by creating an instance and callingthe broadcast
//  routine.
//
//----------------------------------------------------------------------------------------------------------------------
