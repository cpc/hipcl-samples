...
void mpiInit(int *argc, char ***argv) {
  ...
  { // limit scope
    ...
    if ((required > MPI_THREAD_SINGLE) && (required > provided)) {
      printf("MPI-OpenMP Error.\n\tCode requires %s thread support. MPI "
             "library provides %s support.\n",
...
void mpiScan(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
             MPI_Op operation, MPI_Comm comm) {
  qs_assert(MPI_Scan(sendbuf, recvbuf, count, datatype, operation, comm) ==
            MPI_SUCCESS);
}
void mpiType_commit(MPI_Datatype *datatype) {
  qs_assert(MPI_Type_commit(datatype) == MPI_SUCCESS);
}
void mpiType_contiguous(int count, MPI_Datatype old_type,
                        MPI_Datatype *newtype) {
  qs_assert(MPI_Type_contiguous(count, old_type, newtype) == MPI_SUCCESS);
}
void mpiWaitall(int count, MPI_Request *array_of_requests,
                MPI_Status *array_of_statuses) {
  qs_assert(MPI_Waitall(count, array_of_requests, array_of_statuses) ==
            MPI_SUCCESS);
}
void mpiAllreduce(void *sendbuf, void *recvbuf, int count,
                  MPI_Datatype datatype, MPI_Op operation, MPI_Comm comm) {
  qs_assert(MPI_Allreduce(sendbuf, recvbuf, count, datatype, operation, comm) ==
            MPI_SUCCESS);
}
void mpiIAllreduce(void *sendbuf, void *recvbuf, int count,
                   MPI_Datatype datatype, MPI_Op operation, MPI_Comm comm,
                   MPI_Request *request)
#ifdef HAVE_ASYNC_MPI
{
  qs_assert(MPI_Iallreduce(sendbuf, recvbuf, count, datatype, operation, comm,
                           request) == MPI_SUCCESS);
}
#else
{
...
#endif
void mpiReduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
               MPI_Op op, int root, MPI_Comm comm) {
  qs_assert(MPI_Reduce(sendbuf, recvbuf, count, datatype, op, root, comm) ==
            MPI_SUCCESS);
}
void mpiGather(void *sendbuf, int sendcount, MPI_Datatype sendtype,
               void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
               MPI_Comm comm) {
  qs_assert(MPI_Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount,
                       recvtype, root, comm) == MPI_SUCCESS);
}
void mpiBcast(void *buf, int count, MPI_Datatype datatype, int root,
              MPI_Comm comm) {
  qs_assert(MPI_Bcast(buf, count, datatype, root, comm) == MPI_SUCCESS);
}
void mpiIrecv(void *buf, int count, MPI_Datatype datatype, int source, int tag,
              MPI_Comm comm, MPI_Request *request) {
  qs_assert(MPI_Irecv(buf, count, datatype, source, tag, comm, request) ==
            MPI_SUCCESS);
}
void mpiRecv(void *buf, int count, MPI_Datatype datatype, int source, int tag,
             MPI_Comm comm, MPI_Status *status) {
  qs_assert(MPI_Recv(buf, count, datatype, source, tag, comm, status) ==
            MPI_SUCCESS);
}
void mpiIsend(void *buf, int count, MPI_Datatype datatype, int dest, int tag,
              MPI_Comm comm, MPI_Request *request) {
  qs_assert(MPI_Isend(buf, count, datatype, dest, tag, comm, request) ==
            MPI_SUCCESS);
}
void mpiSend(void *buf, int count, MPI_Datatype datatype, int dest, int tag,
             MPI_Comm comm) {
...
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
#else // HAVE_MPI is not defined : Serial (non-MPI) implementation of necessary
      // routines
      // -------------------------------------------------------------------------------
      // -------------------------------------------------------------------------------
...
void mpiReduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
  ...
  case MPI_UNSIGNED_LONG_LONG:
    ...
    break;
  default:
    printf("%s:%d - MPI_Reduce type (%d) not implemented.", __FILE__, __LINE__,
           datatype);
    qs_assert(false);
  }
}
...
void mpiAllreduce(void *sendbuf, void *recvbuf, int count,
  ...
  case MPI_UNSIGNED_LONG_LONG:
    ...
    break;
  default:
    printf("%s:%d - MPI_Allreduce type (%d) not implemented.", __FILE__,
           __LINE__, datatype);
    qs_assert(false);
  }
}
...
void mpiScan(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
  ...
  case MPI_UNSIGNED_LONG_LONG:
    ...
    break;
  default:
    printf("%s:%d - MPI_Scan type (%d) not implemented.", __FILE__, __LINE__,
           datatype);
    qs_assert(false);
  }
}
...
void mpiGather(void *sendbuf, int sendcount, MPI_Datatype sendtype,
  ...
  if (sendcount != recvcount) {
    printf("%s:%d - MPI_Gather sendcount=%d != recvcount=%d\n", __FILE__,
           __LINE__, sendcount, recvcount);
    qs_assert(false);
  }

  if (sendtype != recvtype) {
    printf("%s:%d - MPI_Gather sendtype=%d != recvtype=%d\n", __FILE__,
           __LINE__, sendtype, recvtype);
    qs_assert(false);
  }
  ...
  case MPI_UNSIGNED_LONG_LONG:
    ...
    break;
  default:
    printf("%s:%d - MPI_Gather type (%d) not implemented.", __FILE__, __LINE__,
           recvtype);
    qs_assert(false);
  }
}
...
static Handleitem *init_block(int block, Handleitem *b) {
  ...
  b[MPI_STUBS_BLOCK_ITEMS - 1].handle =
      MPI_STUBS_HANDLE(block, MPI_STUBS_BLOCK_ITEMS - 1);
...
static void init_handles() {
  ...
  for (int i = 1; i < MPI_STUBS_MAX_BLOCKS; i++) {
    mpi_stubs_data.blocks[i] = NULL;
  }
...
static void mpi_stubs_alloc_handle(int *handle, void **data) {
  ...
  if (mpi_stubs_data.need_to_init) {
    init_handles();
  }
