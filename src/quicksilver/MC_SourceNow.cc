...
void MC_SourceNow(MonteCarlo *monteCarlo) {
  ...
  std::vector<double> source_rate(
      monteCarlo->_materialDatabase->_mat.size()); // Get this from user input

  for (int material_index = 0;
       material_index < monteCarlo->_materialDatabase->_mat.size();
       material_index++) {
    std::string name =
        monteCarlo->_materialDatabase->_mat[material_index]._name;
  ...
  for (int domain_index = 0; domain_index < monteCarlo->domain.size();
       domain_index++) {
    MC_Domain &domain = monteCarlo->domain[domain_index];

    for (int cell_index = 0; cell_index < domain.cell_state.size();
         cell_index++) {
      MC_Cell_State &cell = domain.cell_state[cell_index];
      double cell_weight_particles = cell._volume *
                                     source_rate[cell._material] *
                                     monteCarlo->time_info->time_step;
  ...
  mpiAllreduce(&local_weight_particles, &total_weight_particles, 1, MPI_DOUBLE,
               MPI_SUM, monteCarlo->processor_info->comm_mc_world);
  ...
  uint64_t processing_index =
      monteCarlo->_particleVaultContainer->sizeProcessing() / vault_size;

  uint64_t task_index = 0;
  uint64_t particle_count = 0;

  // Compute the partial sums on each mpi process.
  // uint64_t local_num_particles = (int)(local_weight_particles /
  ...
  for (int domain_index = 0; domain_index < monteCarlo->domain.size();
    ...
    for (int cell_index = 0; cell_index < domain.cell_state.size();
      ...
      double cell_weight_particles = cell._volume *
                                     source_rate[cell._material] *
                                     monteCarlo->time_info->time_step;
      double cell_num_particles_float =
      ...
      for (int particle_index = 0; particle_index < cell_num_particles;
           particle_index++) {
        ...
        particle.random_number_seed =
            rngSpawn_Random_Number_Seed(&random_number_seed);
        particle.identifier = random_number_seed;

        MCT_Generate_Coordinate_3D_G(&particle.random_number_seed, domain_index,
                                     cell_index, particle.coordinate,
                                     monteCarlo);

        particle.direction_cosine.Sample_Isotropic(
            &particle.random_number_seed);

        // sample energy uniformly from [eMin, eMax] MeV
        particle.kinetic_energy = (monteCarlo->_params.simulationParams.eMax -
        ...
        particle.time_to_census =
            monteCarlo->time_info->time_step * randomNumber;

        MC_Base_Particle base_particle(particle);

        monteCarlo->_particleVaultContainer->addProcessingParticle(
            base_particle, processing_index);

        particle_count++;

        ATOMIC_UPDATE(
            monteCarlo->_tallies->_balanceTask[particle.task]._source);
      }
    }
  }
