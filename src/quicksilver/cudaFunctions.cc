...
int ThreadBlockLayout(dim3 &grid, dim3 &block, int num_particles) {
  ...
  uint64_t num_blocks = num_particles / threads_per_block +
                        ((num_particles % threads_per_block == 0) ? 0 : 1);
  ...
  if (num_blocks == 0) {
    ...
    printf("Error: num_blocks exceeds maximum block specifications. Cannot "
