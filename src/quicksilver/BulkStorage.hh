...
template <typename T> class BulkStorage {
public:
  BulkStorage()
      : _bulkStorage(0), _refCount(0), _size(0), _capacity(0),
  ...
  BulkStorage(const BulkStorage &aa)
      : _bulkStorage(aa._bulkStorage), _refCount(aa._refCount), _size(aa._size),
        _capacity(aa._capacity), _memPolicy(aa._memPolicy) {
    ++(*_refCount);
  }
