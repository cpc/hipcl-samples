...
//----------------------------------------------------------------------------------------------------------------------
//  Allocate a contiguous particle buffer, set pointers to int, float and char
//  data.
//----------------------------------------------------------------------------------------------------------------------
void particle_buffer_base_type::Allocate(int buffer_size) {
  // we add 2 ints: 1 for the number of particles and the second int is so the
  // float_data buffer will be 8 byte aligned

  uint64_t length_int_data =
      (MC_Base_Particle::num_base_ints * buffer_size + 2) * (int)sizeof(int);
  uint64_t length_float_data =
      (MC_Base_Particle::num_base_floats * buffer_size) * (int)sizeof(double);
  uint64_t length_char_data =
      (MC_Base_Particle::num_base_chars * buffer_size) * (int)sizeof(char);
  ...
  if (length_int_data % sizeof(double) != 0) {
    MC_Fatal_Jump("\nThe particle buffer for floating point data is not 8-byte "
                  "alligned.\n"
  ...
  // Initialize int_index to 2 so can store num_particles in index 0, nothing at
  // index 1.
...
void particle_buffer_base_type::Initialize_Buffer() {
  ...
  // int_index is 2 because num_particles goes in 0 position when the buffer is
  // sent, and position 1 is blank so float_data is 8 byte aligned.
  this->int_index = 2;
  this->float_index = 0;
  this->char_index = 0;
  this->int_data =
      NULL; // Initialize to NULL so can be Allocated in Buffer_Particle
...
void particle_buffer_base_type::Reset_Offsets() {
  ...
  uint64_t length_int_data =
      (MC_Base_Particle::num_base_ints * num_particles + 2) * (int)sizeof(int);
  uint64_t length_float_data =
      (MC_Base_Particle::num_base_floats * num_particles) * (int)sizeof(double);
...
void mcp_test_done_class::Zero_Out() {
  ...
  this->non_blocking_sum[1] =
      1; // initialize these so they are not-equal, [0] != [1]
...
//----------------------------------------------------------------------------------------------------------------------
void mcp_test_done_class::Free_Memory() { this->Zero_Out(); }

//----------------------------------------------------------------------------------------------------------------------
//  Get the number of particles created and completed.
//----------------------------------------------------------------------------------------------------------------------
void mcp_test_done_class::Get_Local_Gains_And_Losses(MonteCarlo *monteCarlo,
                                                     int64_t sent_recv[2]) {
  uint64_t gains = 0, losses = 0;

  Balance &bal =
      monteCarlo->_tallies
          ->_balanceTask[0]; // SumTasks has been called, so just use index 0
...
//----------------------------------------------------------------------------------------------------------------------
//  Initializes the particle buffers, mallocing them and assigning processors to
//  buffers.
//
//----------------------------------------------------------------------------------------------------------------------
void MC_Particle_Buffer::Instantiate() {
  ...
  MC_CALLOC(this->task[0].send_buffer, this->num_buffers,
            particle_buffer_base_type);
  MC_CALLOC(this->task[0].recv_buffer, this->num_buffers,
            particle_buffer_base_type);
...
void MC_Particle_Buffer::Initialize_Map() {
  ...
  for (int domain_index = 0; domain_index < mcco->domain.size();
       domain_index++) {
    MC_Domain &domain = mcco->domain[domain_index];
    for (int neighbor_index = 0; neighbor_index < domain.mesh._nbrRank.size();
         neighbor_index++) {
...
void MC_Particle_Buffer::Unpack_Particle_Buffer(int buffer_index,
  ...
  particle_buffer_base_type &recv_buffer =
      this->task[0].recv_buffer[buffer_index];

  // Get the number of particles in the buffer
  recv_buffer.num_particles = recv_buffer.int_data[int_index++];
  int_index++; // increment past the second integer, (for 8 byte allignment of
               // float_data)
  ...
  if (mcco->_params.simulationParams.debugThreads >= 2) {
    fprintf(stderr,
            "%02d-%02d <- %02d %3d particles "
            "MC_Particle_Buffer::Unpack_Particle_Buffer into vault %d\n",
            mcco->processor_info->rank, omp_get_thread_num(),
            recv_buffer.processor, recv_buffer.num_particles, 0);
  }

  // Unpack each particle and place into a partivault.
  for (int particle_index = 0; particle_index < recv_buffer.num_particles;
       particle_index++) {
    base_particle.Serialize(recv_buffer.int_data, recv_buffer.float_data,
                            recv_buffer.char_data, int_index, float_index,
                            char_index, MC_Data_Member_Operation::Unpack);
...
void MC_Particle_Buffer::Delete_Completed_Extra_Send_Buffers() {
  std::list<particle_buffer_base_type>::iterator it =
      this->task[0].extra_send_buffer.begin();
...
int MC_Particle_Buffer::Choose_Buffer(int processor) {
  ...
  if (buffer < 0 || buffer >= this->num_buffers) {
    MC_Fatal_Jump("Bad buffer value (buffer = %i) for neighbor.next_send %i, "
                  "neighbor.replication_level %i, processor %i\n",
                  buffer, neighbor.task[0].next_send,
                  neighbor.replication_level, processor);
  }
...
int MC_Particle_Buffer::Get_Processor_Buffer_Index(int processor) {
  ...
  if (it == this->processor_buffer_map.end()) {
    // return -1 if the input processor does not communicate with this
    // processor. i.e. not found in map.
    return -1;
  } else {
    return (*it).second;
  }
...
//----------------------------------------------------------------------------------------------------------------------
//  Wrapper routine converts MC_Particle to MC_Base_Particle and calls base
//  Particle version
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
void MC_Particle_Buffer::Buffer_Particle(MC_Base_Particle &particle,
                                         int buffer) {
  particle_buffer_base_type &send_buffer = this->task[0].send_buffer[buffer];

  if (mcco->_params.simulationParams.debugThreads >= 3) {
    fprintf(stderr,
            "%02d-%02d MC_Particle_Buffer::Buffer_Particle entered "
            "task_index=%d buffer_size=%d "
  ...
  if (send_buffer.int_data == NULL) {
    fprintf(stderr,
            "Should not reach here. This should be already preallocated\n");
    send_buffer.Allocate(this->buffer_size);
  }

  // Put the particle into the buffer.
  particle.Serialize(send_buffer.int_data, send_buffer.float_data,
                     send_buffer.char_data, send_buffer.int_index,
                     send_buffer.float_index, send_buffer.char_index,
...
void MC_Particle_Buffer::Send_Particle_Buffer(int buffer) {
  ...
  if (send_buffer.num_particles > 0) {
    ...
    if (mcco->_params.simulationParams.debugThreads >= 2) {
      fprintf(stderr,
              "%02d-%02d -> %02d %3d particles "
              "MC_Particle_Buffer::Send_Particle_Buffer\n",
              mcco->processor_info->rank, omp_get_thread_num(),
              send_buffer.processor, send_buffer.num_particles);
    }

    mpiIsend(send_buffer.int_data, send_buffer.length, MPI_BYTE,
             send_buffer.processor, MC_Tag_Particle_Buffer,
             mcco->processor_info->comm_mc_world, &send_buffer.request_list);

    // non-blocking send, copy send_buffer to the extra list, so we can re-use
    // send_buffer
...
void MC_Particle_Buffer::Post_Receive_Particle_Buffer(size_t bufferSize_) {
  for (int buffer_index = 0; buffer_index < this->num_buffers; buffer_index++) {
    ...
    mpiIrecv(recv_buffer.int_data, recv_buffer.length, MPI_BYTE,
             recv_buffer.processor, MC_Tag_Particle_Buffer,
             mcco->processor_info->comm_mc_world, &recv_buffer.request_list);
  }
}

//----------------------------------------------------------------------------------------------------------------------
//  Receives a particle buffer and puts the particles in it into particle vault
//  to be processed.
//----------------------------------------------------------------------------------------------------------------------
...
bool MC_Particle_Buffer::Test_Done_New(
    MC_New_Test_Done_Method::Enum test_done_method) {
  if (!(mcco->processor_info->num_processors > 1)) {
...
//----------------------------------------------------------------------------------------------------------------------
//  Perform blocking allreduce on particle counts, return true if counts are
//  equal.
//----------------------------------------------------------------------------------------------------------------------
bool MC_Particle_Buffer::Allreduce_ParticleCounts() {
  ...
  mpiAllreduce(buf, hard_blocking_sum, 2, MPI_INT64_T, MPI_SUM,
...
//----------------------------------------------------------------------------------------------------------------------
//  Perform non-blocking allreduce on particle counts, return true if local
//  counts are equal.
//----------------------------------------------------------------------------------------------------------------------
bool MC_Particle_Buffer::Iallreduce_ParticleCounts() {
  ...
  if (flag) {
    if (this->test_done.non_blocking_sum[0] ==
        this->test_done.non_blocking_sum[1]) {
      bool answer = this->Allreduce_ParticleCounts();
      return answer;
    } else {
      this->test_done.Get_Local_Gains_And_Losses(
          mcco, this->test_done.non_blocking_send);
      mpiIAllreduce(this->test_done.non_blocking_send,
                    this->test_done.non_blocking_sum, 2, MPI_INT64_T, MPI_SUM,
                    mcco->processor_info->comm_mc_world,
...
void MC_Particle_Buffer::Free_Memory() {
  ...
  if (this->task) {
    ...
    for (int buffer_index = 0; buffer_index < this->num_buffers;
         buffer_index++) {
  ...
  if (this->task) {
    ...
    for (std::list<particle_buffer_base_type>::iterator it =
             mytask.extra_send_buffer.begin();
