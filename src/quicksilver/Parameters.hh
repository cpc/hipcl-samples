...
struct GeometryParameters {
  ...
  GeometryParameters()
      : materialName(), shape(UNDEFINED), radius(0.0), xCenter(0.0),
        yCenter(0.0), zCenter(0.0), xMin(0.0), yMin(0.0), zMin(0.0), xMax(0.0),
        yMax(0.0), zMax(0.0){};
...
struct MaterialParameters {
  MaterialParameters()
      : name(), mass(1000.0), totalCrossSection(1.0), nIsotopes(10),
        nReactions(9), sourceRate(0.0), scatteringCrossSection(),
        absorptionCrossSection(), fissionCrossSection(),
        scatteringCrossSectionRatio(1.0), absorptionCrossSectionRatio(1.0),
...
struct CrossSectionParameters {
  CrossSectionParameters()
      : name(), aa(0.0), bb(0.0), cc(0.0), dd(0.0), ee(1.0), nuBar(2.4){};
...
struct SimulationParameters {
  SimulationParameters()
      : inputFile(), crossSectionsOut(""), boundaryCondition("reflect"),
        energySpectrum(""), loadBalance(0), cycleTimers(0), debugThreads(0),
        nParticles(1000000),              // 10^6
        batchSize(0),                     // default to use nBatches
        nBatches(10), nSteps(10), nx(10), // speed up early testing
        ny(10), nz(10),
        //     nx(100),
        //     ny(100),
        //     nz(100),
        seed(1029384756), xDom(0), yDom(0), zDom(0), dt(1e-8), fMax(0.1),
        lx(100.0), ly(100.0), lz(100.0), eMin(1e-9), eMax(20), nGroups(230),
        lowWeightCutoff(0.001), balanceTallyReplications(1),
        fluxTallyReplications(1), cellTallyReplications(1), coralBenchmark(0){};

  std::string inputFile;      //!< name of input file
  std::string energySpectrum; //!< enble computing and printing energy spectrum
                              //!< via of energy spectrum file
  std::string crossSectionsOut;  //!< enable or disable printing cross section
                                 //!< data to a file
  ...
  int balanceTallyReplications;  //!< Number of replications for the balance
                                 //!< tallies
  int fluxTallyReplications;     //!< Number of replications for the scalar flux
                                 //!< tally
  int cellTallyReplications;     //!< Number of replications for the scalar cell
                                 //!< tally
