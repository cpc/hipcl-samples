...
public:
  ...
  enum Enum { Undefined = 0, Scatter, Absorption, Fission };

  NuclearDataReaction(){};

  NuclearDataReaction(Enum reactionType, double nuBar,
  ...
  HOST_DEVICE_CUDA
  void sampleCollision(double incidentEnergy, double material_mass,
                       double *energyOut, double *angleOut, int &nOut,
                       uint64_t *seed, int max_production_size);

  qs_vector<double>
      _crossSection;  //!< tabular data for microscopic cross section
...
public:
  void addReaction(NuclearDataReaction::Enum type, double nuBar,
                   qs_vector<double> &energies, const Polynomial &polynomial,
...
public:
  NuclearDataIsotope() : _species(1, VAR_MEM) {}
...
public:
  ...
  int addIsotope(int nReactions, const Polynomial &fissionFunction,
  ...
  HOST_DEVICE_CUDA
  double getReactionCrossSection(unsigned int reactIndex,
                                 unsigned int isotopeIndex, unsigned int group);
