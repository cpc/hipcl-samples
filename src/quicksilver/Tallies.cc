...
void Tallies::SumTasks(void) {
  for (int replication_index = 1; replication_index < _num_balance_replications;
       replication_index++) {
    _balanceTask[0].Add(
        _balanceTask[replication_index]); // Add index 1 and greater to index 0
    _balanceTask[replication_index]
        .Reset(); // reset index 1 and greater after addition
  }
}

void Tallies::CycleFinalize(MonteCarlo *monteCarlo) {
  SumTasks(); // sum the task level data down to index 0 at the end of each
              // cycle
  ...
  mpiAllreduce(&tal[0], &sum[0], tal.size(), MPI_UINT64_T, MPI_SUM,
  ...
  for (int domainIndex = 0; domainIndex < _scalarFluxDomain.size();
       domainIndex++) {
    // Sum Cell Tally Replications
    for (int replication_index = 1; replication_index < _num_flux_replications;
         replication_index++) {
      _cellTallyDomain[domainIndex]._task[0].Add(
          _cellTallyDomain[domainIndex]._task[replication_index]);
    ...
    for (int replication_index = 1; replication_index < _num_flux_replications;
         replication_index++) {
      _scalarFluxDomain[domainIndex]._task[0].Add(
          _scalarFluxDomain[domainIndex]._task[replication_index]);
...
void Fluence::compute(int domainIndex, ScalarFluxDomain &scalarFluxDomain) {
  ...
  for (int cellIndex = 0; cellIndex < numCells; cellIndex++) {
    ...
    for (int groupIndex = 0; groupIndex < numGroups; groupIndex++) {
      fluenceDomain->addCell(
          cellIndex,
          scalarFluxDomain._task[0]._cell[cellIndex]._group[groupIndex]);
    }
  }
}
...
void Tallies::PrintSummary(MonteCarlo *monteCarlo) {
  MC_FASTTIMER_STOP(
      MC_Fast_Timer::cycleFinalize); // stop the finalize timer to get report

  if (monteCarlo->time_info->cycle == 0) {
    Print0("%-8s ", "cycle");
    _balanceTask[0].PrintHeader();
    Print0("%14s %14s %14s %14s\n", "scalar_flux", "cycleInit", "cycleTracking",
           "cycleFinalize");
  }
  ...
  MC_FASTTIMER_START(
      MC_Fast_Timer::cycleFinalize); // restart the finalize timer
}
...
double Tallies::ScalarFluxSum(MonteCarlo *monteCarlo) {
  ...
  for (int domainIndex = 0; domainIndex < _scalarFluxDomain.size();
       domainIndex++) {
    for (int replication_index = 0; replication_index < _num_flux_replications;
         replication_index++) {
      int numCells =
          _scalarFluxDomain[domainIndex]._task[replication_index]._cell.size();

      for (int cellIndex = 0; cellIndex < numCells; cellIndex++) {
        int numGroups = _scalarFluxDomain[domainIndex]
                            ._task[replication_index]
                            ._cell[cellIndex]
                            .size();
        for (int groupIndex = 0; groupIndex < numGroups; groupIndex++) {
          local_sum += _scalarFluxDomain[domainIndex]
                           ._task[replication_index]
                           ._cell[cellIndex]
                           ._group[groupIndex];
        }
  ...
  double sum = 0.0;
  mpiAllreduce(&local_sum, &sum, 1, MPI_DOUBLE, MPI_SUM,
               monteCarlo->processor_info->comm_mc_world);
...
void Tallies::InitializeTallies(MonteCarlo *monteCarlo,
  ...
  if (_cellTallyDomain.size() == 0) {
    ...
    for (int domainIndex = 0; domainIndex < monteCarlo->domain.size();
         domainIndex++) {
      _cellTallyDomain.push_back(CellTallyDomain(
          &monteCarlo->domain[domainIndex], _num_cellTally_replications));
    }
  ...
  if (_scalarFluxDomain.size() == 0) {
    ...
    for (int domainIndex = 0; domainIndex < monteCarlo->domain.size();
         domainIndex++) {
      _scalarFluxDomain.push_back(
          ScalarFluxDomain(&monteCarlo->domain[domainIndex],
