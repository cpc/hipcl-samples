...
void initializeCentersRandomly(int nCenters, const GlobalFccGrid &grid,
                               vector<MC_Vector> &centers);
void initializeCentersGrid(double lx, double ly, double lz, int xDom, int yDom,
                           int zDom, vector<MC_Vector> &centers);
...
void initNuclearData(MonteCarlo *monteCarlo, const Parameters &params) {
  ...
  monteCarlo->_nuclearData = new (ptr1)
      NuclearData(params.simulationParams.nGroups, params.simulationParams.eMin,
  ...
  for (auto crossSectionIter = params.crossSectionParams.begin();
    ...
    crossSection.insert(
        make_pair(cp.name, Polynomial(cp.aa, cp.bb, cp.cc, cp.dd, cp.ee)));
  }
  ...
  for (auto matIter = params.materialParams.begin();
    ...
    for (int iIso = 0; iIso < mp.nIsotopes; ++iIso) {
      int isotopeGid = monteCarlo->_nuclearData->addIsotope(
          mp.nReactions, crossSection.at(mp.fissionCrossSection),
          crossSection.at(mp.scatteringCrossSection),
          crossSection.at(mp.absorptionCrossSection), nuBar,
          mp.totalCrossSection, mp.fissionCrossSectionRatio,
          mp.scatteringCrossSectionRatio, mp.absorptionCrossSectionRatio);
  ...
  for (int iDomain = 0; iDomain < nDomains; ++iDomain) {
    ...
    for (unsigned iCell = 0; iCell < nCells; ++iCell) {
      for (unsigned iFacet = 0; iFacet < 24; ++iFacet) {
        ...
        const Subfacet_Adjacency &backside = domain[jDomain]
                                                 .mesh._cellConnectivity[jCell]
                                                 ._facet[jFacet]
                                                 .subfacet;
...
void initMesh(MonteCarlo *monteCarlo, const Parameters &params) {
  ...
  for (unsigned ii = 0; ii < myDomainGid.size(); ++ii) {
    ...
    monteCarlo->domain.push_back(MC_Domain(
        partition[ii], globalGrid, ddc, params, *monteCarlo->_materialDatabase,
...
void initTallies(MonteCarlo *monteCarlo, const Parameters &params) {
  monteCarlo->_tallies->InitializeTallies(
      monteCarlo, params.simulationParams.balanceTallyReplications,
...
void initializeCentersRandomly(int nCenters, const GlobalFccGrid &grid,
  ...
  set<Tuple> picked;
  do {
    Tuple iTuple(drand48() * grid.nx() / 2, drand48() * grid.ny() / 2,
                 drand48() * grid.nz() / 2);
...
void checkCrossSections(MonteCarlo *monteCarlo, const Parameters &params) {
  ...
  for (unsigned iMat = 0; iMat < nMaterials; ++iMat) {
    ...
    for (unsigned iIso = 0; iIso < nIsotopes; ++iIso) {
      int isotopeGid =
          monteCarlo->_materialDatabase->_mat[iMat]._iso[iIso]._gid;
      unsigned nReactions =
          nd->_isotopes[isotopeGid]._species[0]._reactions.size();
      // for each reaction
      for (unsigned iReact = 0; iReact < nReactions; ++iReact) {
        // loop over energies
        NuclearDataReaction &reaction =
            nd->_isotopes[isotopeGid]._species[0]._reactions[iReact];
        ...
        for (unsigned iGroup = 0; iGroup < nGroups; ++iGroup) {
          ...
          case NuclearDataReaction::Scatter:
            xcVec[iGroup].scatter +=
                reaction.getCrossSection(iGroup) / nIsotopes;
            break;
          case NuclearDataReaction::Absorption:
            xcVec[iGroup].absorption +=
                reaction.getCrossSection(iGroup) / nIsotopes;
            break;
          case NuclearDataReaction::Fission:
            xcVec[iGroup].fission +=
                reaction.getCrossSection(iGroup) / nIsotopes;
            break;
  ...
  std::string fileName =
      monteCarlo->_params.simulationParams.crossSectionsOut + ".dat";
  ...
  for (auto mapIter = xcTable.begin(); mapIter != xcTable.end(); ++mapIter) {
    ...
    fprintf(xSec, "  %s_a  %s_f  %s_s", materialName.c_str(),
            materialName.c_str(), materialName.c_str());
  }
  fprintf(xSec, "\n");
  ...
  for (unsigned ii = 0; ii < nGroups; ++ii) {
    ...
    for (auto mapIter = xcTable.begin(); mapIter != xcTable.end(); ++mapIter) {
      fprintf(xSec, "  %g  %g  %g", mapIter->second[ii].absorption,
              mapIter->second[ii].fission, mapIter->second[ii].scatter);
    }
    fprintf(xSec, "\n");
  }
  fclose(xSec);
