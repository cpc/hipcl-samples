...
public:
  enum Enum { Count = 0, Pack = 1, Unpack = 2, Reset = 3 };
};
...
//
// This public method "invalidates" this particle by setting its particle type
// to UNKNOWN. This method will fail if this particle is already invalid.
//
...
inline MC_Base_Particle::MC_Base_Particle()
    : coordinate(), velocity(), kinetic_energy(0.0), weight(0.0),
      time_to_census(0.0), age(0.0), num_mean_free_paths(0.0),
      num_segments(0.0), random_number_seed((uint64_t)0),
      identifier((uint64_t)0), last_event(MC_Tally_Event::Census),
      num_collisions(0), breed(0),
      // species == -1 is a special signifier for invalidated particle
      species(-1), domain(0), cell(0) {}
...
HOST_DEVICE_CUDA
inline MC_Base_Particle &MC_Base_Particle::
operator=(const MC_Particle &particle) {
  coordinate = particle.coordinate;
  velocity = particle.velocity;
  kinetic_energy = particle.kinetic_energy;
  weight = particle.weight;
...
inline MC_Particle::MC_Particle()
    : coordinate(), velocity(), direction_cosine(), kinetic_energy(0.0),
      weight(0.0), time_to_census(0.0), totalCrossSection(0.0), age(0.0),
      num_mean_free_paths(0.0), mean_free_path(0.0), segment_path_length(0.0),
      random_number_seed((uint64_t)0), identifier((uint64_t)0),
      last_event(MC_Tally_Event::Census), num_collisions(0), num_segments(0.0),

      task(0), species(0), breed(0), energy_group(0), domain(0), cell(0),
      facet(0), normal_dot(0.0) {}
HOST_DEVICE_END
...
inline MC_Particle::MC_Particle(const MC_Base_Particle &from_particle)
    : coordinate(from_particle.coordinate), velocity(from_particle.velocity),
      ...
      time_to_census(from_particle.time_to_census), age(from_particle.age),
      num_mean_free_paths(from_particle.num_mean_free_paths),

      mean_free_path(0.0), segment_path_length(0.0),
      ...
      task(0), species(from_particle.species), breed(from_particle.breed),
      energy_group(0), domain(from_particle.domain), cell(from_particle.cell),
      normal_dot(0.0) {
...
//----------------------------------------------------------------------------------------------------------------------
inline void MC_Base_Particle::Copy_Particle_Base_To_String(
