...
public:
  enum Enum { Send, Ssend, Bsend, Rsend, Isend, Issend, Ibsend, Irsend };
};
...
public:
  particle_buffer_task_class()
      : send_buffer(NULL), recv_buffer(NULL), extra_send_buffer() {}
  particle_buffer_base_type *send_buffer; // send particle buffers
  particle_buffer_base_type *recv_buffer; // recv particle buffers
  std::list<particle_buffer_base_type>
      extra_send_buffer; // extra send buffers for non-blocking sends
};
...
public:
  enum Enum { AllProcessorTree, Blocking, NonBlocking };
};
...
private:
  ...
  std::map<int, int>
      processor_buffer_map; // Map processors to buffers. buffer_index =
                            // processor_buffer_map[processor]
...
public:
  ...
  bool Test_Done_New(MC_New_Test_Done_Method::Enum test_done_method =
...
private:
  ...
  MC_Particle_Buffer &
  operator=(const MC_Particle_Buffer &tmp); // disable assignment operator
};
...
/*
  ...
  char_index, length .int_data, float_data, char_data .request_list
