...
void printParameters(const Parameters &pp, ostream &out) {
  ...
  for (map<string, MaterialParameters>::const_iterator iter =
           pp.materialParams.begin();
       iter != pp.materialParams.end(); ++iter)
    out << iter->second;

  for (map<string, CrossSectionParameters>::const_iterator iter =
           pp.crossSectionParams.begin();
...
void parseCommandLine(int argc, char **argv, Parameters &pp) {
  ...
  addArg("fMax", 'f', 1, 'd', &(sp.fMax), 0,
         "max random mesh node displacement");
  addArg("inputFile", 'i', 1, 's', &(name), sizeof(name), "name of input file");
  addArg("energySpectrum", 'e', 1, 's', &(esName), sizeof(esName),
         "name of energy spectrum output file");
  addArg("crossSectionsOut", 'S', 1, 's', &(xsec), sizeof(xsec),
         "name of cross section output file");
  addArg("loadBalance", 'l', 0, 'i', &(sp.loadBalance), 0,
         "enable/disable load balancing");
  addArg("cycleTimers", 'c', 1, 'i', &(sp.cycleTimers), 0,
         "enable/disable cycle timers");
  addArg("debugThreads", 't', 1, 'i', &(sp.debugThreads), 0,
         "set thread debug level to 1, 2, 3");
  ...
  addArg("batchSize", 'g', 1, 'u', &(sp.batchSize), 0,
         "number of particles in a vault/batch");
  addArg("nBatches", 'b', 1, 'u', &(sp.nBatches), 0,
         "number of vault/batch to start (sets batchSize automaticaly)");
  ...
  addArg("bTally", 'B', 1, 'i', &(sp.balanceTallyReplications), 0,
         "number of balance tally replications");
  addArg("fTally", 'F', 1, 'i', &(sp.fluxTallyReplications), 0,
         "number of scalar flux tally replications");
  addArg("cTally", 'C', 1, 'i', &(sp.cellTallyReplications), 0,
...
void scanMaterialBlock(const InputBlock &input, Parameters &pp) {
  ...
  input.getValue<double>("absorptionCrossSectionRatio",
                         mp.absorptionCrossSectionRatio);
  input.getValue<string>("fissionCrossSection", mp.fissionCrossSection);
  input.getValue<double>("fissionCrossSectionRatio",
  ...
  input.getValue<double>("scatteringCrossSectionRatio",
