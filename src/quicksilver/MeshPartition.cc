...
void assignCellsToDomain(MeshPartition::MapType &domainMap,
                         vector<int> &nbrDomains, int myDomainGid,
                         const vector<MC_Vector> &domainCenter,
                         const GlobalFccGrid &grid);

void buildCellIndexMap(MeshPartition::MapType &cellIndexMap, int myDomainGid,
                       int myForeman, int myDomainIndex,
                       const vector<int> &nbrDomains, const GlobalFccGrid &grid,
                       CommObject *comm);

void addNbrsToFlood(Long64 iCell, const GlobalFccGrid &grid,
                    queue<Long64> &floodQueue, set<Long64> &wetCells);

} // namespace

MeshPartition::MeshPartition(int domainGid, int domainIndex, int foreman)
    : _domainGid(domainGid), _domainIndex(domainIndex), _foreman(foreman) {}
...
void MeshPartition::buildMeshPartition(const GlobalFccGrid &grid,
                                       ...
                                       CommObject *comm) {
  assignCellsToDomain(_cellInfoMap, _nbrDomains, _domainGid, centers, grid);

  buildCellIndexMap(_cellInfoMap, _domainGid, _foreman, _domainIndex,
                    _nbrDomains, grid, comm);
}
...
void assignCellsToDomain(MeshPartition::MapType &assignedDomainMap,
  ...
  for (auto iter = remoteDomainSet.begin(); iter != remoteDomainSet.end();
       ++iter) {
    nbrDomains[ind++] = *iter;
  }
}
...
namespace {
void buildCellIndexMap(MeshPartition::MapType &cellInfoMap, int myDomainGid,
                       int myForeman, int myDomainIndex,
                       const vector<int> &nbrDomain, const GlobalFccGrid &grid,
                       CommObject *comm) {
  int nLocalCells = 0;
