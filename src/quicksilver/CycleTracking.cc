...
HOST_DEVICE
void CycleTrackingGuts(MonteCarlo *monteCarlo, int particle_index,
  ...
  // set the particle.task to the index of the processed vault the particle will
  // census into.
  mc_particle.task = 0; // processed_vault;

  // loop over this particle until we cannot do anything more with it on this
  // processor
  CycleTrackingFunction(monteCarlo, mc_particle, particle_index,
...
HOST_DEVICE
void CycleTrackingFunction(MonteCarlo *monteCarlo, MC_Particle &mc_particle,
                           int particle_index, ParticleVault *processingVault,
                           ParticleVault *processedVault) {
  ...
  do {
    ...
    monteCarlo->_tallies->TallyCellValue(
        exp(rngSample(&mc_particle.random_number_seed)), mc_particle.domain,
        cell_tally_index, mc_particle.cell);
#endif
