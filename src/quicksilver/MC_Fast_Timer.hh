...
public:
  ...
  std::chrono::high_resolution_clock::time_point
      startClock; // from c++11 high resolution timer calls
  ...
  MC_Fast_Timer()
      : numCalls(0), startClock(), stopClock(), lastCycleClock(0),
        cumulativeClock(0){}; // consturctor
...
public:
  ...
  void Cumulative_Report(int mpi_rank, int num_ranks, MPI_Comm comm_world,
                         uint64_t numSegments);
  void Last_Cycle_Report(int report_time, int mpi_rank, int num_ranks,
                         MPI_Comm comm_world);
  void Clear_Last_Cycle_Timers();
  MC_Fast_Timer
      timers[MC_Fast_Timer::Num_Timers]; // timers for various routines
...
#ifdef DISABLE_TIMERS // Disable timers with empty macros -- do not make timer
                      // calls
...
#ifdef CHRONO_MISSING // compiler does not support high resolution timer, use
                      // MPI timer instead
...
#define MC_FASTTIMER_STOP(timerIndex)                                          \
  if (omp_get_thread_num() == 0) {                                             \
    ...
    mcco->fast_timer->timers[timerIndex].lastCycleClock +=                     \
        (long unsigned)((mcco->fast_timer->timers[timerIndex].stopClock -      \
                         mcco->fast_timer->timers[timerIndex].startClock) *    \
                        1000000.0);                                            \
...
#define MC_FASTTIMER_GET_LASTCYCLE(timerIndex)                                 \
  (float)mcco->fast_timer->timers[timerIndex].lastCycleClock / 1000000.

#else // else CHRONO_MISSING is not defined, so high resolution clock is
      // available
...
#define MC_FASTTIMER_STOP(timerIndex)                                          \
  if (omp_get_thread_num() == 0) {                                             \
    mcco->fast_timer->timers[timerIndex].stopClock =                           \
        std::chrono::high_resolution_clock::now();                             \
    mcco->fast_timer->timers[timerIndex].lastCycleClock +=                     \
