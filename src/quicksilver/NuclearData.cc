...
// 1MeV.
NuclearDataReaction::NuclearDataReaction(Enum reactionType, double nuBar,
                                         const qs_vector<double> &energies,
                                         const Polynomial &polynomial,
                                         double reactionCrossSection)
    : _crossSection(energies.size() - 1, 0., VAR_MEM),
      _reactionType(reactionType), _nuBar(nuBar) {
...
void NuclearDataReaction::sampleCollision(double incidentEnergy,
                                          double material_mass,
                                          double *energyOut, double *angleOut,
                                          int &nOut, uint64_t *seed,
                                          int max_production_size) {
  double randomNumber;
...
void NuclearDataSpecies::addReaction(NuclearDataReaction::Enum type,
                                     double nuBar, qs_vector<double> &energies,
                                     const Polynomial &polynomial,
                                     double reactionCrossSection) {
  _reactions.Open();
  _reactions.push_back(NuclearDataReaction(type, nuBar, energies, polynomial,
                                           reactionCrossSection));
  _reactions.Close();
}

// Set up the energies boundaries of the neutron
NuclearData::NuclearData(int numGroups, double energyLow, double energyHigh)
    : _energies(numGroups + 1, VAR_MEM) {
...
int NuclearData::addIsotope(int nReactions, const Polynomial &fissionFunction,
                            const Polynomial &scatterFunction,
                            const Polynomial &absorptionFunction, double nuBar,
                            double totalCrossSection, double fissionWeight,
                            double scatterWeight, double absorptionWeight) {
  _isotopes.Open();
...
HOST_DEVICE
double NuclearData::getTotalCrossSection(unsigned int isotopeIndex,
                                         unsigned int group) {
  qs_assert(isotopeIndex < _isotopes.size());
...
// Return the total cross section for this energy group
HOST_DEVICE
double NuclearData::getReactionCrossSection(unsigned int reactIndex,
                                            unsigned int isotopeIndex,
                                            unsigned int group) {
  qs_assert(isotopeIndex < _isotopes.size());
