...
// Currently not atomic here. But its only used when it does not necissarially
// need to be atomic.
...
#define ATOMIC_CAPTURE(x, v, p) p = atomicAdd(&x, v);
// If in a CPU OpenMP section use the OpenMP atomics
#elif defined(USE_OPENMP_ATOMICS)
#define ATOMIC_WRITE(x, v) _Pragma("omp atomic write") x = v;

#define ATOMIC_ADD(x, v) _Pragma("omp atomic") x += v;

#define ATOMIC_UPDATE(x) _Pragma("omp atomic update") x++;
...
#define ATOMIC_UPDATE(x) x++;

#define ATOMIC_ADD(x, v) x += v;
