...
void chemv_gpu(float alpha_re, float alpha_im, float beta_re, float beta_im,
...

    {
      ...
      hipCheckReturn(hipMalloc((void **) &dev_AT, (136900) * sizeof(struct ComplexFloat)));
      hipCheckReturn(hipMalloc((void **) &dev_X, (370) * sizeof(struct ComplexFloat)));
      hipCheckReturn(hipMalloc((void **) &dev_Y, (370) * sizeof(struct ComplexFloat)));
      
      hipCheckReturn(hipMemcpy(dev_AT, AT, (136900) * sizeof(struct ComplexFloat), hipMemcpyHostToDevice));
      hipCheckReturn(hipMemcpy(dev_X, X, (370) * sizeof(struct ComplexFloat), hipMemcpyHostToDevice));
      hipCheckReturn(hipMemcpy(dev_Y, Y, (370) * sizeof(struct ComplexFloat), hipMemcpyHostToDevice));
      {
        dim3 k0_dimBlock(32);
        dim3 k0_dimGrid(12);
        hipLaunchKernelGGL(kernel0, dim3(k0_dimGrid), dim3(k0_dimBlock), 0, 0, dev_AT, dev_X, dev_Y, alpha_im, alpha_re, beta_im, beta_re);
        hipCheckKernel();
      }
      {
        dim3 k1_dimBlock(32);
        dim3 k1_dimGrid(12);
        hipLaunchKernelGGL(kernel1, dim3(k1_dimGrid), dim3(k1_dimBlock), 0, 0, dev_AT, dev_X, dev_Y, alpha_im, alpha_re);
        hipCheckKernel();
      }
      
      hipCheckReturn(hipMemcpy(Y, dev_Y, (370) * sizeof(struct ComplexFloat), hipMemcpyDeviceToHost));
      hipCheckReturn(hipFree(dev_AT));
      hipCheckReturn(hipFree(dev_X));
      hipCheckReturn(hipFree(dev_Y));
    }
}
...
int main() {
  ...
  for (int i = 0; i < N; i++)
    // The precision may be adjusted for a target device, but the results
    // should match exactly if they are converted to integers.
