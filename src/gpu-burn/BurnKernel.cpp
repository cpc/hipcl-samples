...
BurnKernel::BurnKernel(int hipDevice)
    : mHipDevice(hipDevice), mRunKernel(false), mDeviceAdata(NULL),
      mDeviceBdata(NULL), mDeviceCdata(NULL) {}
...
extern "C" __global__ void hip_sgemm_kernel(const int M, const int N,
...
int BurnKernel::Init() {
  ...
  std::string msg =
      "Init Burn Thread for device (" + std::to_string(hipDevice) + ")\n";
  std::cout << msg;
  ...
  checkError(
      hipMalloc((void **)&mDeviceCdata, matrixSizeBytes * mNumIterations),
      "Alloc C");

  checkError(hipMemcpy(mDeviceAdata, mHostAdata, matrixSizeBytes,
                       hipMemcpyHostToDevice),
             "A -> device");
  checkError(hipMemcpy(mDeviceBdata, mHostBdata, matrixSizeBytes,
                       hipMemcpyHostToDevice),
             "B -> device");
  checkError(hipMemset(mDeviceCdata, 0, matrixSizeBytes * mNumIterations),
             "C memset");
...
int BurnKernel::threadMain() {
  ...
  int hipDevice = bindHipDevice();
  std::string msg =
      "Burn Thread using device (" + std::to_string(hipDevice) + ")\n";
  std::cout << msg;
...
int BurnKernel::stopBurn() {
  ...
  std::string msg =
      "Stopping burn thread on device (" + std::to_string(hipDevice) + ")\n";
  std::cout << msg;
...
int BurnKernel::runComputeKernel() {
  ...
  for (int i = 0; mRunKernel && i < mNumIterations; ++i) {
    hipLaunchKernelGGL(
        /* Launch params */
        hip_sgemm_kernel, dim3(cRowSize / cBlockSize, cRowSize / cBlockSize, 1),
        dim3(cBlockSize, cBlockSize, 1), 0, 0,
        /* Kernel params */
        cRowSize, cRowSize, cRowSize, cAlpha, mDeviceAdata, cRowSize,
        mDeviceBdata, cRowSize, cBeta, mDeviceCdata + i * cMatrixSize,
        cRowSize);
  }
