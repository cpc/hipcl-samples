...
static inline void GetDevicePeakInfo(double *aGIPS, double *aGBPS,
                                     hipDeviceProp_t *aDeviceProp = NULL) {
...
static void StoreDeviceInfo(FILE *fout) {
  ...
  // deviceProp.memoryClockRate/1000/2); fprintf(fout, "Memory bus width:    %d
  ...
  fprintf(
      fout,
      "Compute throughput:  %.2f GFlops (theoretical single precision FMAs)\n",
      2.0 * InstrThroughput);
