...
#pragma acc data copy(                                                         \
    rho [0:sizex * sizey * Nmats], p [0:sizex * sizey * Nmats],                \
    t [0:sizex * sizey * Nmats], Vf [0:sizex * sizey * Nmats])                 \
    copy(V [0:sizex * sizey], x [0:sizex * sizey], y [0:sizex * sizey],        \
         n [0:Nmats], rho_ave [0:sizex * sizey])                               \
      ...
      for (int i = 0; i < sizex; i++) {
        ...
        for (int mat = 0; mat < Nmats; mat++) {
          ...
          if (Vf[(i + sizex * j) * Nmats + mat] > 0.0)
            ave += rho[(i + sizex * j) * Nmats + mat] *
                   Vf[(i + sizex * j) * Nmats + mat];
        }
...
#pragma acc loop independent
        ...
        for (int mat = 0; mat < Nmats; mat++) {
          if (Vf[(i + sizex * j) * Nmats + mat] > 0.0) {
            double nm = n[mat];
            p[(i + sizex * j) * Nmats + mat] =
                (nm * rho[(i + sizex * j) * Nmats + mat] *
                 t[(i + sizex * j) * Nmats + mat]) /
                Vf[(i + sizex * j) * Nmats + mat];
          } else {
            ...
            for (int nj = -1; nj <= 1; nj++) {
              ...
              for (int ni = -1; ni <= 1; ni++) {
                ...
                if (Vf[((i + ni) + sizex * (j + nj)) * Nmats + mat] > 0.0) {
                  rho_sum += rho[((i + ni) + sizex * (j + nj)) * Nmats + mat] /
                             dsqr[(nj + 1) * 3 + (ni + 1)];
                  Nn += 1;
                }
              }
...
#pragma acc loop independent
        ...
        for (int i = 0; i < sizex; i++) {
          ...
          if (Vf[ncells * mat + i + sizex * j] > 0.0)
            rho_ave[i + sizex * j] += rho[ncells * mat + i + sizex * j] *
                                      Vf[ncells * mat + i + sizex * j];
        }
      }
    }
...
#pragma acc loop independent
        ...
        for (int i = 0; i < sizex; i++) {
          double nm = n[mat];
          if (Vf[ncells * mat + i + sizex * j] > 0.0) {
            p[ncells * mat + i + sizex * j] =
                (nm * rho[ncells * mat + i + sizex * j] *
                 t[ncells * mat + i + sizex * j]) /
                Vf[ncells * mat + i + sizex * j];
          } else {
            ...
            for (int nj = -1; nj <= 1; nj++) {
              ...
              for (int ni = -1; ni <= 1; ni++) {
                ...
                if (Vf[ncells * mat + (i + ni) + sizex * (j + nj)] > 0.0) {
                  ...
                  rho_sum +=
                      rho[ncells * mat + (i + ni) + sizex * (j + nj)] / dsqr;
                  Nn += 1;
                }
              }
            }
...
bool full_matrix_check_results(full_data cc, full_data mc) {
  ...
  for (int j = 0; j < sizey; j++) {
    for (int i = 0; i < sizex; i++) {
      if (fabs(cc.rho_ave[i + sizex * j] - mc.rho_ave[i + sizex * j]) >
          0.0001) {
        printf("1. cell-centric and material-centric values are not equal! "
               "(%f, %f, %d, %d)\n",
      ...
      for (int mat = 0; mat < Nmats; mat++) {
        if (fabs(cc.p[(i + sizex * j) * Nmats + mat] -
                 mc.p[ncells * mat + i + sizex * j]) > 0.0001) {
          printf("2. cell-centric and material-centric values are not equal! "
                 "(%f, %f, %d, %d, %d)\n",
                 cc.p[(i + sizex * j) * Nmats + mat],
                 mc.p[ncells * mat + i + sizex * j], i, j, mat);
          return false;
        }

        if (fabs(cc.rho_mat_ave[(i + sizex * j) * Nmats + mat] -
                 mc.rho_mat_ave[ncells * mat + i + sizex * j]) > 0.0001) {
          printf("3. cell-centric and material-centric values are not equal! "
                 "(%f, %f, %d, %d, %d)\n",
                 cc.rho_mat_ave[(i + sizex * j) * Nmats + mat],
                 mc.rho_mat_ave[ncells * mat + i + sizex * j], i, j, mat);
          return false;
        }
      }
    }
