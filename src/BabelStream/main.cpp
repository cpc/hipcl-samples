...
template <typename T> void run() {
  ...
  size_t sizes[5] = {2 * sizeof(T) * ARRAY_SIZE, 2 * sizeof(T) * ARRAY_SIZE,
...
template <typename T> void run_triad() {
  ...
  if (!output_as_csv) {
    ...
    if (mibibytes) {
      std::cout << std::setprecision(1) << std::fixed
                << "Array size: " << ARRAY_SIZE * sizeof(T) * pow(2.0, -10.0)
                << " KiB"
                << " (=" << ARRAY_SIZE * sizeof(T) * pow(2.0, -20.0) << " MiB)"
                << std::endl;
      std::cout << "Total size: "
                << 3.0 * ARRAY_SIZE * sizeof(T) * pow(2.0, -10.0) << " KiB"
                << " (=" << 3.0 * ARRAY_SIZE * sizeof(T) * pow(2.0, -20.0)
                << " MiB)" << std::endl;
    } else {
      std::cout << std::setprecision(1) << std::fixed
                ...
                << std::endl;
      std::cout << "Total size: " << 3.0 * ARRAY_SIZE * sizeof(T) * 1.0E-3
                << " KB"
  ...
  double bandwidth =
      ((mibibytes) ? pow(2.0, -30.0) : 1.0E-9) * (total_bytes / runtime);
  ...
  if (output_as_csv) {
    ...
    std::cout << "Triad" << csv_separator << num_times << csv_separator
              << ARRAY_SIZE << csv_separator << sizeof(T) << csv_separator
              << bandwidth << csv_separator << runtime << std::endl;
  } else {
    std::cout << "--------------------------------" << std::endl
              << std::fixed << "Runtime (seconds): " << std::left
              << std::setprecision(5) << runtime << std::endl
...
void check_solution(const unsigned int ntimes, std::vector<T> &a,
  ...
  double errA =
      std::accumulate(a.begin(), a.end(), 0.0, [&](double sum, const T val) {
        return sum + fabs(val - goldA);
      });
  errA /= a.size();
  double errB =
      std::accumulate(b.begin(), b.end(), 0.0, [&](double sum, const T val) {
        return sum + fabs(val - goldB);
      });
  errB /= b.size();
  double errC =
      std::accumulate(c.begin(), c.end(), 0.0, [&](double sum, const T val) {
        return sum + fabs(val - goldC);
      });
  errC /= c.size();
